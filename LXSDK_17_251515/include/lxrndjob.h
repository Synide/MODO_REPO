...
typedef struct vt_ILxRenderSystemListener ** ILxRenderSystemListenerID;
 ...
 * longer needed.
 * 
 * Return whether or not the current job is running in worker mode.
 * 
 ...
 * 
 * There are a maximum number of render slots.  Not all slots may be filled, and
 * there may be gaps, but this is the maximum number of slots that has been set
 * in the prefs.  It is actually possible for more older frame buffers to exist
 * on disk with higher slot numbers, but modo won't create slot numbers beyond
 * this maximum.
 * 
 */
...
/*
 * The maximum number of render slots has changed.
 */
typedef struct vt_ILxRenderSystemListener {
	ILxUnknown	 iunk;
	LXxMETHOD( void,
MaxRenderSlots) (
	LXtObjectID		 self,
	int			 maxSlots);
} ILxRenderSystemListener;
...
RenderProcessAllocate) (
	LXtObjectID			  self,
	int				  bufferIndex,
	LXtRenderOutputProcess	**rndrProcess );
	LXxMETHOD(  LxResult,
 ...
 */
	...
	#define LXe_RENDER_METAL_CONTEXT	LXxFAILCODE( LXeSYS_RENDER, 13)
	#define LXe_RENDER_METAL_SCENE		LXxFAILCODE( LXeSYS_RENDER, 14)
	#define LXe_RENDER_METAL_LAUNCH		LXxFAILCODE( LXeSYS_RENDER, 15)
 ...
 * will be called for both a local render and worker mode.  Notably, the progress
 * methods will not be called in worker mode:
 * - RenderItem()
 * - ActionName()
 * - GroupName()
 * - RenderAs()
...
/*
 * Clients can use ILxRenderSystemListener for events related to the system as
 * a whole, but not necessarily about in-progrees renderings.
 */

	#define LXu_RENDERSYSTEMLISTENER			"4c8551af-515b-4983-9edd-70e37a1870d6"
	// [local]  ILxRenderSystemListener
	// [export] ILxRenderSystemListener rndsys
