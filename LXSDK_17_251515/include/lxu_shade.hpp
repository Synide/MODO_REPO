...
class CLxPacketWrapper
{
	...
	CLxPacketWrapper ()		{ m_pkt = nullptr; }
		...
		public CLxVectorPacket_Void
{
	...
	void	v_init (void *p) override
	{
		init ((P *) p);
	}

	void	v_cleanup (void *p) override
	{
		cleanup ((P *) p);
	}

	void	v_reset (void *p) override
	{
		reset ((P *) p);
	}

	void	v_copy (void *p, void *f) override
	{
		copy ((P *) p, (const P *) f);
	}

	void	v_blend (void *p, void *p0, void *p1, float t, int mode) override
	{
		blend ((P *) p, (const P *) p0, (const P *) p1, t, mode);
	}

	void	v_invert (void *p) override
	{
		invert ((P *) p);
	}
};
