 ...
 * the graph.
 * 
 * The scene service can be used to create an ILxEvaluator, given an active ILxEvaluation object.
 * 
 ...
 * 
 * Returns the index of the cinema.
 * 
 * 
 * 
 */
 ...
 * 
 * Returns the number of user channels on an item. User channels always come last,
 * so the user channels can be enumerated starting at ChannelCount - UserChannelCount.
 * 
 ...
 * 
 * Duplicate a channel to a new item
 */
 ...
 */
	...
	// [const]  ILxItem:UserChannelCount
	// [const]  ILxItem:Ident
...
/*
 * Returns LXe_TRUE if the scene is changing. Clients should avoid modifying the
 * scene or evaluating scene state when it is in this state. An optional argument
 * describes the changes that are being performed.
 * - LOADING
 * If the loading flag is set the scene is loading.
 * - REMODEL
 * If this flag is set, the state of the scene may be unusual, and clients should
 * not correct anything odd they find.
 * 
 */

	#define LXf_SCENECHANGE_LOADING	 0x01
	#define LXf_SCENECHANGE_REMODEL	 0x02
