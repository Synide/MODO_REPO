...
namespace lx {
		...
		unsigned		 count)
	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			weight[i] = that->GetWeight (v, (points ? points[i] : nullptr), (polygons ? polygons[i] : nullptr));
		}
		...
		public CLxMetaObject
{
	...
	{
	    public:
		float vf_weight (const LXtFVector pos, const LXtPointID point, const LXtPolygonID polygon) override
		{
			T		*t = (T *) this;
			return t->weight_world (pos, point, polygon);
		}

		LxResult vf_run (const float **pos, const LXtPointID *points, const LXtPolygonID *polygons, float *weight, unsigned num) override
		{
			T		*t = (T *) this;
			...
			public CLxExtendedPolymorph<F>
	{
		...
		CLxGenericPolymorph *
	new_spawner (
		CLxMetaInterfaces	*ifcs)	override
	{
		if (is_local)
