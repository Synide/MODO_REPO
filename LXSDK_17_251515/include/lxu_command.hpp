		...
		public CLxObject
{
	 ...
	 */
	virtual const char *	button_name () { return nullptr; }

	/*
	 * Implement to return a custom icon.
	 */
	virtual const char *	icon_name () { return nullptr; }
	 ...
	 */
	CLxUser_Message &	cmd_message (const char * = nullptr);

	/*
	 * Call to raise an error with optional message key. This function
	 * never returns.
	 */
	void			cmd_error (LxResult = LXe_FAILED, const char * = nullptr);
		...
		public CLxMeta_Notifier_Core
{
	...
	E		 cur_event;
		...
		void
	visit_raw (
		CLxNotifier_Core	*noti)			override
	{
		...
		public CLxDynamicArguments
{
	...
	LxResult	cmd_Enable	(ILxUnknownID msg)				override;
	LxResult	cmd_Flags	(unsigned int *flags)				override;
	LxResult	cmd_ArgFlags	(unsigned int index, unsigned int *flags)	override;
	LxResult	cmd_ArgClear	(unsigned int index)				override;
	LxResult	cmd_ArgResetAll	()						override;
	LxResult	cmd_ArgSetDatatypes ()						override;
	LxResult	cmd_ArgEnable	(unsigned int arg)				override;
	LxResult	cmd_Message	(void **ppvObj)					override;
	LxResult	cmd_ButtonName	(const char **name)				override;
	LxResult	cmd_Icon	(const char **name)				override;
	void		cmd_Interact	()						override;
	void		cmd_Execute	(unsigned flags)				override;

	LxResult	cmd_NotifyAddClient	(int argument, ILxUnknownID object)	override;
	LxResult	cmd_NotifyRemoveClient	(ILxUnknownID object)			override;
 ...
 *	initialize ()
 *	{
 *		CLxExport_BasicCommand<CMyCommand>::Define("myCommandName");
 *	}
 */
template <class T>
		...
		public CLxImpl_Notifier
{
	...
	LxResult		noti_Name         (const char **name)   override;
	LxResult		noti_SetArgs      (const char  *args)   override;
	LxResult		noti_Args         (const char **args)   override;
	LxResult		noti_AddClient    (ILxUnknownID object) override;
	LxResult		noti_RemoveClient (ILxUnknownID object) override;
...
class CLxItemTypeOperator
{
	...
	void		op_add_choice (const char *user, const char *internal = nullptr);
