...
typedef struct vt_ILxEvaluator ** ILxEvaluatorID;
 ...
 * 
 * Typically, modifiers only get a call to their Validate function if they are using the 
 * evaluation cache. While this is most efficient, there are cases where receiving 
 * validation calls can be useful for modifiers in order to set related state. For those 
 * cases, clients can call ForceValidations, which (if available) will ensure the modifier
 * receives calls to its Validate function when input channels change.  If force invalidations
 * are not supported for the given ILxEvaluation interface, this returns LXe_NOTAVAILABLE.
 */
...
/*
 * Just as with normal evaluation interfaces, evaluators require their input and output channels to
 * be added by their item and channel index.  The type is either LXfECHAN_READ or LXfECHAN_WRITE, and the
 * returned attr value is the index into the evalutor's array to be used when setting or getting channel values later.
 * After all channels for read and write are added to the evalutor, the evaluator must be marked ready.  This allows the
 * eval system to compute the subsection of the eval graph to use and configure which channel values need to be fetched through
 * the fallback cache.
 * 
 * Set an input float value.
 * 
 * Read an output float value.
 * 
 * Set an input integer value.
 * 
 * Read an output integer value.
 * 
 * Set an input object value.
 * 
 * Read an output object value.
 */
typedef struct vt_ILxEvaluator {
	ILxUnknown	 iunk;
	LXxMETHOD( LxResult,
AddChannel) (
	LXtObjectID		 self,
 ...
 */
	...
	// [default] ILxEvaluation:ForceValidations = LXe_NOTAVAILABLE
...
/*
 * A Subset Evaluator is a means to rapidly evaluate only a subset of the scene's full evaluation graph.  The Evaluator
 * populates values in the evaluation graph with the desired inputs, then evaluates itself to compute values for the requested outputs.  Any
 * channel values being used but not explicitly marked by the user as inputs will use cached values from the evaluation object
 * that was originally used to create the evaluator.  Evaluators are the basis for all "operator" modifiers in modo, allowing per-attribute
 * data to be computed without needed to repeatedly change channel values for inputs.  They are created through the scene service,
 * requiring an ILxEvaluation object to serve as the fallback cache.
 */

	#define LXu_EVALUATOR	"7F4B20C2-5646-475B-906F-FA3CB9026D99"
	#define LXa_EVALUATOR	"evaluator"
	// [export]  ILxEvaluator evaluator
	// [local]   ILxEvaluator
