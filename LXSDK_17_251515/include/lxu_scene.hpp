...
class CLxSceneBuilder
{
	...
	unsigned		 AddXform     (LXtTransformType, unsigned * = nullptr);

	unsigned		 AddPreXform  (LXtTransformType, double[3], unsigned * = nullptr);
	unsigned		 AddPreXform  (LXtTransformType, double, double, double, unsigned * = nullptr);
	unsigned		 AddPreXform  (LXtTransformType, double, unsigned * = nullptr);

	unsigned		 AddPostXform (LXtTransformType, double[3], unsigned * = nullptr);
	unsigned		 AddPostXform (LXtTransformType, double, double, double, unsigned * = nullptr);
	unsigned		 AddPostXform (LXtTransformType, double, unsigned * = nullptr);

	unsigned		 PrependXform (LXtTransformType, double[3], unsigned * = nullptr);
	unsigned		 PrependXform (LXtTransformType, double, double, double, unsigned * = nullptr);
	unsigned		 PrependXform (LXtTransformType, double, unsigned * = nullptr);

	unsigned		 AppendXform  (LXtTransformType, double[3], unsigned * = nullptr);
	unsigned		 AppendXform  (LXtTransformType, double, double, double, unsigned * = nullptr);
	unsigned		 AppendXform  (LXtTransformType, double, unsigned * = nullptr);
	...
	unsigned		 AddImageMap  (const char *, const char * = nullptr, const char * = "Texture", int where = CURMASK);
	...
	bool			 SetMap       (LXtID4, const char *name = nullptr);
...
class CLxSceneLoader : public CLxImpl_Loader
{
	...
	LxResult		 load_Recognize  (const char *, ILxUnknownID)	 override;
	LxResult		 load_LoadObject (ILxUnknownID, ILxUnknownID, ILxUnknownID)
										 override;
	void			 load_Cleanup    ()				 override;
	LxResult		 load_SpawnOptions (void **)			 override;
...
class CLxSceneSaver : public CLxImpl_Saver
{
	...
	LxResult		 sav_Verify   (ILxUnknownID, ILxUnknownID)		 override;
	LxResult		 sav_Save     (ILxUnknownID, const char *, ILxUnknownID) override;
	...
	virtual bool		 ss_ItemValidForExport (CLxUser_Item&) { return true; }
	...
	void			 StartScan    (const char * = nullptr);
	bool			 NextItem     ();
	bool			 NextMesh     ();
	bool			 NextSelectedMesh ();
	bool			 GetMesh      (CLxUser_Mesh &);
	bool			 ScanMask     (const char *, const char * = nullptr);
	bool			 NextLayer    ();
	...
	bool			 PntNormal    (LXtVector, LXtPolygonID = nullptr);
	...
	bool			 PolyNormal   (LXtVector, LXtPointID = nullptr);
