...
typedef struct vt_ILxRenderSlotPacketTranslation ** ILxRenderSlotPacketTranslationID;
 ...
 * 
 * This creates a new packet from a given path and identifier.  This packet
 ...
 * 
 * 
 * Slot selections can be targeted at specific clients, which are identified by a key string.
 * An empty key string denotes the default selection.
 * 
 * 
 * This creates a new packet from a identifier and optional client key.
 * 
 */
typedef struct vt_ILxRenderSlotPacketTranslation {
	ILxUnknown	 iunk;
	LXxMETHOD(  int,
SlotIdent) (
	LXtObjectID		  self,
...
Packet) (
	...
	int			 outputIndex,
	const char		*client);
} ILxRenderSlotPacketTranslation;
	...
	#define LXiSEL_RENDERSLOT	LXxID4('R','S','L','T')	// Render Slot
 ...
 * Render slots are used to decide which frame buffer to display in the render view,
 * stats view, and other related views.  Slots are referenced by a unique identifer
 * (an integer).
 * 
 * The slot packet not only includes a slot identifier, but also a pass index and
 * output index.  This is because the true selection is an output within a slot pass,
 * not the slot itself.  Selecting a slot on its own is functionally identically
 * to selecting the first output in the first pass of the slot.
 * 
 */

	#define LXsSELTYP_RENDERSLOT			"renderslot"
	#define LXu_RENDERSLOTPACKETTRANSLATION		"d6e1af16-e8be-4be2-a4cf-4849a67e5d9c"
	// [local]  ILxRenderSlotPacketTranslation
