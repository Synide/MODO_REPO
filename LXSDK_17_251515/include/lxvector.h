...
typedef struct st_LXpSamplePosition {
	...
	float		*dpduvLen;	// array of UV derivative vector lengths
...
typedef struct st_LXpSampleTransform {
	...
	bool		 hasTransform; // If object has non unit transform
} LXpSampleTransform;
...
typedef struct st_LXpSampleBump {
	...
	LXtVector	 bump;			// this is the result of bump map evaluation: the delta from the normal
	float		 bumpAmplitude;		// bump amplitude as set by the material
	float		 bumpHeight[3];		// bump height at sample position for each facet
	int		 bumpHeightId;		// bump height facet id
	...
	bool	 dispMapping;		// flag set to true during displacement mapping texture evaluation
} LXpSampleBump;
...
typedef struct st_LXpTextureInput {
	...
	LXtFVector	 dpdu, dpdv;    // U,V derivative vectors
...
// added in 902:
	...
	float			 dpduLen, dpdvLen;    // variable-length U,V derivative vector lenghts --Unused--

// added in 16.1:
	bool			 inputCulled; // If this texture input has been culled by projection math skip evaluation
	bool			 triplanarProjection; // Specail case for triplanar projection, skips several blending steps in TLFilter_EvalEffect
} LXpTextureInput;
