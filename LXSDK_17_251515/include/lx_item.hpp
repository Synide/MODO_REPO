...
class CLxUser_Item : public CLxLoc_Item
{
	...
	/**
	 * Returns True or False if the specified channel is a user channel.
	 */
		bool
	IsUserChannel (
		unsigned int		 index)
	{
		...
		return (LXx_FAIL (ChannelPackage (index, &unused)) || NULL == unused);
	}
		...
		const char		*name)
	{
		...
		if (NULL != name && LXx_OK (ChannelLookup (name, &idx)))
			return IsUserChannel (idx);
	...
	NUserChannels () const
	{
		...
		if (LXx_OK (UserChannelCount (&count)))
			return count;
