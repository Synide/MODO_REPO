		...
		public CLxImpl_Modifier
{
	...
	LxResult	 mod_Evaluate ()					override;
	LxResult	 mod_Test (ILxUnknownID, unsigned)			override;
	LxResult	 mod_Validate (ILxUnknownID, unsigned, LxResult)	override;
	void		 mod_Free (void *)					override;
};
		...
		public CLxImpl_TagDescription
{
	...
	LxResult	 eval_Reset (ILxUnknownID scene)		override;
	LXtObjectID	 eval_Next (unsigned *index)			override;
	LxResult	 eval_Alloc (ILxUnknownID item, unsigned index, ILxUnknownID eval, void **ppvObj)
									override;

	unsigned	 tag_Count ()					override;
	LxResult	 tag_Describe (unsigned, LXtTagInfoDesc *desc)	override;
 ...
 *	initialize ()
 *	{
 *		CLxExport_ItemModifierServer<CMyModifier>::Define("myModifier");
 *	}
 */
template <class T>
		...
		public CLxItemModifierElement
{
	...
	void		 Eval (CLxUser_Evaluation &, CLxUser_Attributes &)		override;
};
