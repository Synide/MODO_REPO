...
class CLxGenericPolymorph
{
	 ...
	 */
	ILxUnknownID		 Spawn (const LXtGUID *iid = nullptr);
	 ...
	 */
    //internal:
				 CLxGenericPolymorph () : ifc_count (0), id (nullptr) {}
	virtual			~CLxGenericPolymorph ();
	virtual ILxUnknownID	 IsA (ILxUnknownID, const LXtGUID* /*iid*/) { return nullptr; }
	virtual void *		 NewObj  ()       = 0;
	virtual void		 FreeObj (void *obj) = 0;
	virtual void *		 RefObj  (void* /*obj*/) { return nullptr; }
...
#define LXxSINGLETON_METHOD	void * NewObj() override { return this; }
		 ...
		 public CLxSpawnerCommon<T>
{
	...
	CLxSpawner ()
	{
		CLxSpawnerCommon<T>::spawn = nullptr;
	}
...
#define LXx_ADD_SERVER(what,T,name)\
	{ CLxGenericPolymorph *_macro_add_server_srv = new CLxPolymorph<T>;\
	_macro_add_server_srv->AddInterface (new CLxIfc_##what<T>);\
	_macro_add_server_srv->AddInterface (new CLxIfc_StaticDesc<T>);\
	lx::AddServer (name, _macro_add_server_srv); }
