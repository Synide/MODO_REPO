...
public:
    ...
    unsigned col_buf()
    {
        unsigned a = static_cast<unsigned>(LXxMAX(col_rgb.length(), col_alpha.length()));
        unsigned b = static_cast<int>(LXxMAX(alt_rgb.length(), alt_alpha.length()));
        unsigned c = LXxMAX(a, b);
        return static_cast<unsigned>(LXxMAX(c, col_swap.length()));
    }
};
...
void CLxUIValue::popup_clear()
{
    pv->pop_list.clear();
    pv->pop_cur = nullptr;
}

void CLxUIValue::popup_add(const char* value, const char* name, const char* tooltip)
{
...
void CLxUIValue::popup_flags(int flags)
{
    pv->pop_cur->flags = flags;
}

void CLxUIValue::popup_resource(const char* resrc)
{
    pv->pop_cur->resrc.assign(resrc);
}

void CLxUIValue::popup_set_alphasort(bool state)
{
    pv->alpha_sort = state;
}

void CLxUIValue::popup_set_filter(bool state)
{
    pv->filter = state;
}

void CLxUIValue::popup_set_category(const char* cat)
{
    pv->category.assign(cat);
}

void CLxUIValue::popup_set_imagesize(int w, int h)
{
    pv->size_w   = w;
    pv->size_h   = h;
    pv->has_size = true;
...
void CLxUIValue::color_set(const char* rgb, const char* alpha)
{
    pv->col_rgb   = rgb;
    pv->col_alpha = alpha;
}

void CLxUIValue::color_alt(const char* rgb, const char* alpha)
{
    pv->alt_rgb   = rgb;
    pv->alt_alpha = alpha;
}

void CLxUIValue::color_swap(const char* swap)
{
    pv->col_swap = swap;
}

void CLxUIValue::notify_add(const char* name, const char* arg)
{
    std::string tmp;
...
void CLxUIValue::form_add_cmd(const char* cmd)
{
    std::string tmp(cmd);
...
void CLxUIValue::set_cue_text(const char* text)
{
    pv->cue_text.assign(text);
}

unsigned CLxUIValue::Flags()
{
    unsigned flag = 0;
...
unsigned CLxUIValue::PopCount()
{
    return static_cast<unsigned>(pv->pop_list.size());
}

const char* CLxUIValue::PopUserName(unsigned index)
{
    return pv->pop_list[index].user.get_opt();
}

const char* CLxUIValue::PopInternalName(unsigned index)
{
    return pv->pop_list[index].internal.get_opt();
}

const char* CLxUIValue::PopToolTip(unsigned index)
{
    return pv->pop_list[index].tooltip.get_opt();
}

int CLxUIValue::PopFlags(unsigned index)
{
    return pv->pop_list[index].flags;
}

LxResult CLxUIValue::PopIconResource(int index, const char** iconResource)
{
...
const char* CLxUIValue::PopCategory()
{
    return pv->category.get_opt();
}

LxResult CLxUIValue::PopIconSize(int& w, int& h)
{
    if (!pv->has_size)
        return LXe_NOTIMPL;
...
LxResult CLxUIValue::ColorPickerCommands(char* rgb, char* alpha, char* rgbAlt, char* alphaAlt, char* useAlt, unsigned bufLens)
{
...
int CLxUIValue::NotifierCount()
{
    return static_cast<int>(pv->note_cmd.size());
}

LxResult CLxUIValue::NotifierByIndex(int index, const char** name, const char** args)
{
...
int CLxUIValue::FormCommandListCount()
{
    return static_cast<int>(pv->form_cmd.size());
}

const char* CLxUIValue::FormCommandListByIndex(int index)
{
    return pv->form_cmd[index].c_str();
}

const char* CLxUIValue::CueText()
{
 ...
 */
class CUIValueHintWrapper : public CLxImpl_UIValueHints
{
public:
    ...
    unsigned uiv_Flags(void) override
    {
        return val->Flags();
    }

    unsigned uiv_PopCount(void) override
    {
        return val->PopCount();
    }

    const char* uiv_PopUserName(unsigned index) override
    {
        if (index > val->PopCount())
            return "**OOB**";
        return val->PopUserName(index);
    }

    const char* uiv_PopInternalName(unsigned index) override
    {
        if (index > val->PopCount())
            return "**OOB**";
    ...
    const char* uiv_PopToolTip(unsigned index) override
    {
        if (index > val->PopCount())
            return "**OOB**";
    ...
    LxResult uiv_PopFlags(unsigned index, int* flags) override
    {
        if (index > val->PopCount())
            return LXe_OUTOFBOUNDS;
    ...
    LxResult uiv_PopCategory(const char** category) override
    {
        category[0] = val->PopCategory();
        return LXe_OK;
    }

    LxResult uiv_PopIconSize(int* w, int* h) override
    {
        return val->PopIconSize(*w, *h);
    }

    LxResult uiv_PopIconImage(int index, void** ppvObj) override
    {
        if (index > val->PopCount())
            return LXe_OUTOFBOUNDS;
    ...
    LxResult uiv_PopIconResource(int index, const char** iconResource) override
    {
        if (index > val->PopCount())
            return LXe_OUTOFBOUNDS;
    ...
    LxResult uiv_ItemTest(ILxUnknownID item) override
    {
        CLxUser_Item ui(item);
        return val->ItemTest(ui) ? LXe_TRUE : LXe_FALSE;
    }

    LxResult uiv_ColorPickerCommands(char* rgb, char* alpha, char* rgbAlt, char* alphaAlt, char* useAlt, unsigned bufLens) override
    {
        return val->ColorPickerCommands(rgb, alpha, rgbAlt, alphaAlt, useAlt, bufLens);
    }

    LxResult uiv_NotifierCount(int* count) override
    {
        *count = val->NotifierCount();
        return (count == nullptr) ? LXe_NOTIMPL : LXe_OK;
    }

    LxResult uiv_NotifierByIndex(int index, const char** name, const char** args) override
    {
        if (index > val->NotifierCount())
            return LXe_OUTOFBOUNDS;
        return val->NotifierByIndex(index, name, args);
    }

    LxResult uiv_FormCommandListCount(int* count) override
    {
        *count = val->FormCommandListCount();
        return (*count == 0) ? LXe_NOTIMPL : LXe_OK;
    }

    LxResult uiv_FormCommandListByIndex(int index, const char** command) override
    {
        if (index > val->FormCommandListCount())
            return LXe_OUTOFBOUNDS;
        *command = val->FormCommandListByIndex(index);
        return (*command == nullptr) ? LXe_FAILED : LXe_OK;
    }

    LxResult uiv_CueText(char* buf, unsigned len) override
    {
        const char* text;
    ...
    LxResult uiv_TextValidate(const char* value, char* buf, unsigned int len) override
    {
 ...
 */
void lx::UIValueHintsAllocate(CLxDynamicUIValue* val, void** ppvObj, bool ownIt)
{
 ...
 */
void CLxDynamicAttributes::dyna_Add(const char* name, const char* type)
{
...
void CLxDynamicAttributes::dyna_Add(std::string name, std::string type)
{
 ...
 */
pv_DynaAttributes::DynaAttr& pv_DynaAttributes::Elt(unsigned int index)
{
    if (index >= dlist.size())
 ...
 */
CLxUser_Value& CLxDynamicAttributes::dyna_Value(unsigned int index)
{
 ...
 */
bool CLxDynamicAttributes::dyna_IsSet(unsigned int index)
{
    return pv->Elt(index).set;
}

void pv_DynaAttributes::Set(CLxDynamicAttributes* dyna, unsigned int index, bool value)
{
    Elt(index).set = value;
    if (value)
 ...
 */
void CLxDynamicAttributes::dyna_SetHint(unsigned int index, const LXtTextValueHint* hint)
{
 ...
 */
void CLxDynamicAttributes::dyna_SetType(unsigned int index, const char* type)
{
 ...
 */
void CLxDynamicAttributes::dyna_SetFlags(unsigned int index, unsigned flags)
{
    pv->Elt(index).flags = flags;
}

unsigned CLxDynamicAttributes::dyna_GetFlags(unsigned int index)
{
 ...
 */
unsigned int CLxDynamicAttributes::attr_Count()
{
    return static_cast<unsigned int>(pv->dlist.size());
}

LxResult CLxDynamicAttributes::attr_Name(unsigned int index, const char** name)
{
    name[0] = pv->Elt(index).name.c_str();
    return LXe_OK;
}

LxResult CLxDynamicAttributes::attr_Type(unsigned int index, unsigned int* type)
{
    type[0] = dyna_Value(index).Type();
    return LXe_OK;
}

LxResult CLxDynamicAttributes::attr_TypeName(unsigned int index, const char** tname)
{
    tname[0] = pv->Elt(index).type.c_str();
    return LXe_OK;
}

LxResult CLxDynamicAttributes::attr_Lookup(const char* name, unsigned int* index)
{
...
const LXtTextValueHint* CLxDynamicAttributes::attr_Hints(unsigned int index)
{
 ...
 * value object.
 */
LxResult CLxDynamicAttributes::attr_Value(unsigned int index, void** ppvObj, int writeOK)
{
    if (writeOK)
        pv->Set(this, index);
...
LxResult CLxDynamicAttributes::attr_GetBool(unsigned int index, bool* val)
{
    int      value;
    LxResult result;
...
LxResult CLxDynamicAttributes::attr_SetBool(unsigned int index, bool val)
{
    int value = val ? 1 : 0;
...
LxResult CLxDynamicAttributes::attr_GetInt(unsigned int index, int* val)
{
    return dyna_Value(index).GetInt(val);
}

LxResult CLxDynamicAttributes::attr_SetInt(unsigned int index, int val)
{
    pv->Set(this, index);
    return dyna_Value(index).SetInt(val);
}

LxResult CLxDynamicAttributes::attr_GetFlt(unsigned int index, double* val)
{
    return dyna_Value(index).GetFlt(val);
}

LxResult CLxDynamicAttributes::attr_SetFlt(unsigned int index, double val)
{
    pv->Set(this, index);
    return dyna_Value(index).SetFlt(val);
}

LxResult CLxDynamicAttributes::attr_GetString(unsigned int index, char* val, unsigned int len)
{
    return dyna_Value(index).GetString(val, len);
}

LxResult CLxDynamicAttributes::attr_SetString(unsigned int index, const char* val)
{
    pv->Set(this, index);
 ...
 */
bool CLxDynamicAttributes::dyna_Bool(unsigned int index, bool value)
{
    if (dyna_IsSet(index))
...
int CLxDynamicAttributes::dyna_Int(unsigned int index, int value)
{
    if (dyna_IsSet(index))
...
double CLxDynamicAttributes::dyna_Float(unsigned int index, double value)
{
    if (dyna_IsSet(index))
...
bool CLxDynamicAttributes::dyna_String(unsigned int index, std::string& text, const char* value)
{
    if (!dyna_IsSet(index))
    {
        text = value;
...
bool CLxDynamicAttributes::dyna_Object(unsigned int index, CLxLocalizedObject& loc)
{
    if (!dyna_IsSet(index))
        return false;
 ...
 * class.
 */
LxResult pv_DynaAttributes::sgs_GetString(char* buf, unsigned len)
{
    return Elt(sgs_index).value.GetString(buf, len);
}

LxResult CLxDynamicAttributes::attr_GetString(unsigned int index, std::string& value)
{
 ...
 */
LxResult CLxDynamicAttributes::atrui_UIHints(unsigned int index, ILxUnknownID hints)
{
...
LxResult CLxDynamicAttributes::atrui_DisableMsg(unsigned int index, ILxUnknownID msg)
{
 ...
 */
LxResult CLxDynamicAttributes::atrui_UIValueHints(unsigned int index, void** ppvObj)
{
    CLxDynamicUIValue* val;
 ...
 */
unsigned CLxDynamicArguments::dyna_GetFlags(unsigned index)
{
    return CLxDynamicAttributes::dyna_GetFlags(index) | (dyna_IsSet(index) ? LXfCMDARG_VALUE_SET : 0);
}
 ...
 */
bool CLxDynamicArguments::dyna_HasVariable()
{
 ...
 */
void CLxDynamicArguments::dyna_SetVariable()
{
    unsigned int i, k;
 ...
 */
void CLxDynamicArguments::dyna_Clear(unsigned index)
{
    unsigned int i, k;

    dyna_SetType(index, nullptr);
...
void CLxDynamicArguments::dyna_ClearAll()
{
    for (int i = 0; i < attr_Count(); i++)
        dyna_Clear(i);
}
 ...
 */
unsigned CLxUIListPopup::Flags()
{
    return LXfVALHINT_POPUPS;
}

unsigned CLxUIListPopup::PopCount()
{
    user_list.clear();
    internal_list.clear();
    tooltip_list.clear();
...
const char* CLxUIListPopup::PopUserName(unsigned index)
{
...
const char* CLxUIListPopup::PopInternalName(unsigned index)
{
...
const char* CLxUIListPopup::PopToolTip(unsigned index)
{
    if (index < tooltip_list.size())
        return tooltip_list[index].empty() ? nullptr : tooltip_list[index].c_str();
