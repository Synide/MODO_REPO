...
unsigned CLxCommand::cmd_exec_flags()
{
    return pv->e_flags;
}

bool CLxCommand::cmd_interaction_ok(bool err)
{
...
void CLxCommand::cmd_read_args(void* ptr)
{
    pv->mpv->attr_desc.read_args(pv->m_dyna[0], ptr);
}

void CLxCommand::cmd_read_args_isset(void* ptr)
{
    pv->mpv->attr_desc.read_args_isset(pv->m_dyna[0], ptr);
}

bool CLxCommand::cmd_read_arg(const char* arg, int& val, int def)
{
...
bool CLxCommand::cmd_read_arg(const char* arg, double& val, double def)
{
    unsigned idx = pv->mpv->attr_desc.by_name(arg);
...
bool CLxCommand::cmd_read_arg(const char* arg, std::string& val, const char* def)
{
    return pv->m_dyna->dyna_String(pv->mpv->attr_desc.by_name(arg), val, def);
}

bool CLxCommand::cmd_read_arg(const char* arg, CLxLocalizedObject& val)
{
    return pv->m_dyna->dyna_Object(pv->mpv->attr_desc.by_name(arg), val);
}

void CLxCommand::cmd_set_arg(const char* arg, int val)
{
    lx_err::check(pv->m_dyna->attr_SetInt(pv->mpv->attr_desc.by_name(arg), val));
}

void CLxCommand::cmd_set_arg(const char* arg, double val)
{
    lx_err::check(pv->m_dyna->attr_SetFlt(pv->mpv->attr_desc.by_name(arg), val));
}

void CLxCommand::cmd_set_arg(const char* arg, const char* val)
{
    lx_err::check(pv->m_dyna->attr_SetString(pv->mpv->attr_desc.by_name(arg), val));
}

void CLxCommand::cmd_set_arg(const char* arg, std::string val)
{
    cmd_set_arg(arg, val.c_str());
}

void CLxCommand::cmd_add_query(int val)
{
    pv->va_query.AddInt(val);
}

void CLxCommand::cmd_add_query(double val)
{
    pv->va_query.AddFloat(val);
}

void CLxCommand::cmd_add_query(const char* val)
{
    pv->va_query.AddString(val);
}

void CLxCommand::cmd_add_query(std::string val)
{
    cmd_add_query(val.c_str());
}

void CLxCommand::cmd_add_query(ILxUnknownID val)
{
    pv->va_query.AddValue(val);
}

void CLxCommand::cmd_add_query(CLxLocalizedObject& val)
{
    pv->va_query.AddEmptyValue(val);
}

CLxDynamicArguments* CLxCommand::cmd_args()
{
    return pv->m_dyna;
}

CLxUser_Message& CLxCommand::cmd_message(const char* key)
{
    pv->init_msg();
...
void CLxCommand::cmd_error(LxResult rc, const char* key)
{
    cmd_message(key);
    throw(rc);
}

void CLxCommand::cmd_add_notifier(const char* name, const char* args)
{
    lx_err::check(pv->notifier_state == -1);
    pv->n_host.add(name, args);
}

void CLxCommand::cmd_add_notifier(std::string name, std::string args)
{
 ...
 */
class impl_Command_Server : public CLxImpl_Command, public CLxDynamicArguments, public CLxImpl_TagDescription
{
public:
    pv_Meta_Command* mpv;
    pv_Command*      ipv;
     ...
     */
    impl_Command_Server(pv_Meta_Command* init = nullptr)
    {
        mpv = init;
        if (lx::IsTempSpawn())
        {
    ...
    const char* dyna_GetType(unsigned index) override
    {
        return mpv->attr_desc.arg_get_type(index, *inst);
    }

    void atrui_UIHints2(unsigned index, CLxUser_UIHints& hints) override
    {
        // This wrapper does not handle the test for any attribute required for Command based hints.
        // If this comes in, bail out, to avoid the throw.
        if (index == static_cast<unsigned int>(LXiATTRUI_ANY))
            return;
        mpv->attr_desc.arg_uihints(index, hints, *inst);
    }

    CLxDynamicUIValue* atrui_UIValue(unsigned index) override
    {
        return mpv->attr_desc.arg_uivalue(index, *inst);
    }

    LxResult enable_result(bool enab)
    {
        if (enab)
            return LXe_OK;
    ...
    LXxO_Command_NotifyAddClient  // (int argument, ILxUnknownID object)
    {
        if (ipv->notifier_state == -2)
        {
            ...
            n = static_cast<int>(mpv->note_names.size());
            for (i = 0; i < n; i += 2)
        ...
        if (argument != -1)
        {
            lx_err::check(ipv->notifier_state == -1);
 ...
 */
CLxMeta_Command_Core::CLxMeta_Command_Core(const char* cmdName) : CLxMetaServer(cmdName)
{
...
void* CLxMeta_Command_Core::alloc()
{
    return reinterpret_cast<void*>(new CLxMetagenPolymorph<impl_Command_Server, pv_Meta_Command>(pv));
}

void CLxMeta_Command_Core::set_type_flags(unsigned flags)
{
    pv->cmd_flags |= flags;
}

void CLxMeta_Command_Core::set_type_model()
{
    set_type_flags(LXfCMD_MODEL | LXfCMD_UNDO);
}

void CLxMeta_Command_Core::set_type_UI()
{
    set_type_flags(LXfCMD_UI);
}

void CLxMeta_Command_Core::add_notifier(const char* name, const char* args)
{
    std::string tmp;
...
public:
    ...
    const char*                 srv_name;
    std::set<CLxNotifier_Core*> n_table;
    ...
    void free_inst(CLxNotifier_Core* noti)
    {
        n_table.erase(noti);
        delete noti;
    }
...
public:
    ...
    std::map<ILxUnknownID, CLxUser_CommandEvent> clients;
...
void CLxNotifier_Core::notify_cmd(unsigned cmdFlags)
{
    for (auto mit = pv->clients.begin(); mit != pv->clients.end(); mit++)
 ...
 * Server implementation class
 */
class impl_Notifier_Server : public CLxImpl_Notifier, public CLxImpl_TagDescription
{
public:
    pv_Meta_Notifier* mpv;
     ...
     * Inialization sets up the arguments from the desc.
     */
    impl_Notifier_Server(pv_Meta_Notifier* init = nullptr)
    {
        mpv    = init;
        p_inst = mpv->get_inst();
    }
 ...
 * Metaclass implementation.
 */
CLxMeta_Notifier_Core::CLxMeta_Notifier_Core(const char* srvName) : CLxMetaServer(srvName)
{
...
void* CLxMeta_Notifier_Core::alloc()
{
    return reinterpret_cast<void*>(new CLxMetagenPolymorph<impl_Notifier_Server, pv_Meta_Notifier>(pv));
}

void CLxMeta_Notifier_Core::enumerate_raw()
{
    for (auto sit = pv->n_table.begin(); sit != pv->n_table.end(); sit++)
        visit_raw(*sit);
}
...
public:
    CLxUser_Message               msg;
    std::vector<CLxUser_Notifier> notifiers;
    std::vector<ILxUnknownID>     noteClients;
    std::string                   bname, icon;
    LxResult                      code;
    bool                          initNotifiers;
 ...
 */
void CLxBasicCommand::basic_SetFlags(unsigned int index, unsigned int flags)
{
 ...
 */
LxResult CLxBasicCommand::cmd_Enable(ILxUnknownID msg)
{
 ...
 */
LxResult CLxBasicCommand::cmd_Flags(unsigned int* flags)
{
 ...
 */
LxResult CLxBasicCommand::cmd_ArgClear(unsigned int index)
{
    dyna_Clear(index);
    return LXe_OK;
}

LxResult CLxBasicCommand::cmd_ArgResetAll()
{
    dyna_ClearAll();
    return LXe_OK;
}
 ...
 */
LxResult CLxBasicCommand::cmd_ArgFlags(unsigned int index, unsigned int* flags)
{
    flags[0] = dyna_GetFlags(index);
    return LXe_OK;
}

LxResult CLxBasicCommand::cmd_ArgSetDatatypes()
{
    dyna_SetVariable();
    return LXe_OK;
}

LxResult CLxBasicCommand::cmd_ArgEnable(unsigned int arg)
{
 ...
 */
LxResult CLxBasicCommand::cmd_ButtonName(const char** name)
{
...
LxResult CLxBasicCommand::cmd_Icon(const char** name)
{
 ...
 */
CLxUser_Message& CLxBasicCommand::basic_Message()
{
    return pv->msg;
}

LxResult CLxBasicCommand::cmd_Message(void** ppvObj)
{
 ...
 */
void CLxBasicCommand::cmd_Execute(unsigned flags)
{
    try
    {
        basic_Execute(flags);
 ...
 */
void CLxBasicCommand::cmd_Interact()
{
    try
    {
        basic_Interact();
 ...
 */
LxResult CLxBasicCommand::cmd_NotifyAddClient(int argument, ILxUnknownID object)
{
    if (pv->initNotifiers)
    {
            ...
            if (LXx_OK(atrui_UIValueHints(argument, &uiValueHintsObj)))
            {
                ...
                if (uiValueHints.take(uiValueHintsObj))
                {
                    if (LXx_OK(uiValueHints.NotifierCount(&n)))
                    {
                        ...
                        {
                            const char *np = nullptr, *ap = nullptr;
                            if (LXx_OK(uiValueHints.NotifierByIndex(index, &np, &ap)) && (np != nullptr))
                            {
                                name = np;
                                args = ap;
                                basic_AddNotify(name, args);
 ...
 */
void CLxBasicCommand::basic_AddNotify(const char* name, const char* args)
{
...
void CLxBasicCommand::basic_AddNotify(const std::string& name, const std::string& args)
{
    basic_AddNotify(name.c_str(), args.c_str());
}

LxResult CLxBasicCommand::cmd_NotifyRemoveClient(ILxUnknownID object)
{
    auto it = pv->noteClients.begin();
    while (1)
    {
 ...
 */
CLxPopupChoiceCommand::CLxPopupChoiceCommand(const char* name, const char* type)
{
    dyna_Add(name, type);
    basic_SetFlags(0, LXfCMDARG_QUERY);
}

LxResult CLxPopupChoiceCommand::cmd_Query(unsigned int index, ILxUnknownID vaQuery)
{
...
void CLxPopupChoiceCommand::popcmd_GetChoices2(std::vector<std::string>& user,
...
class CPCC_Popup : public CLxUIListPopup
{
public:
...
CLxDynamicUIValue* CLxPopupChoiceCommand::atrui_UIValue(unsigned int /*index*/)
{
...
{
public:
    std::set<std::pair<std::string, std::string>> choices;
    bool                                          has_internal;
};

CLxItemTypeOperator::CLxItemTypeOperator(const char* type) : item_sel(type)
{
...
bool CLxItemTypeOperator::op_Notifier(unsigned index, std::string& name, std::string& args)
{
    if (index == 0)
    {
...
bool CLxItemTypeOperator::op_Enable()
{
    CLxUser_Item item;
...
void CLxItemTypeOperator::op_GetCurrent(CLxUser_ValueArray& va)
{
...
void CLxItemTypeOperator::op_GetChoices(std::vector<std::string>& user, std::vector<std::string>& internal)
{
    CLxItemSelection::ItemList     list;
    CLxItemSelection::ItemList_Itr lit;
    ...
    for (auto sit = pv->choices.begin(); sit != pv->choices.end(); sit++)
    {
...
void CLxItemTypeOperator::op_add_choice(const char* user, const char* intr)
{
    std::pair<std::string, std::string> proto;
...
void CLxItemTypeOperator::op_Execute()
{
    CLxItemSelection::ItemList     list;
    CLxItemSelection::ItemList_Itr lit;
...
{
public:
    std::map<ILxUnknownID, CLxUser_CommandEvent> l_client;
    std::string                                  s_name, s_args;
};
 ...
 */
typedef std::set<CLxCommandNotifier*> CommandNotifierSet;

static std::map<std::string, CommandNotifierSet> cn_MasterTable;

CLxCommandNotifier::CLxCommandNotifier(const char* name)
{
...
CLxCommandNotifier::~CLxCommandNotifier()
{
    ...
    {
        auto it = pv->l_client.begin();
        CLxCommandNotifier::noti_RemoveClient(it->first);
    }

    delete pv;
}

LxResult CLxCommandNotifier::noti_Name(const char** name)
{
    name[0] = pv->s_name.c_str();
    return LXe_OK;
}

LxResult CLxCommandNotifier::noti_SetArgs(const char* args)
{
    try
    {
        pv->s_args = args;
...
LxResult CLxCommandNotifier::noti_Args(const char** args)
{
    args[0] = pv->s_args.c_str();
    return LXe_OK;
}

LxResult CLxCommandNotifier::noti_AddClient(ILxUnknownID object)
{
...
LxResult CLxCommandNotifier::noti_RemoveClient(ILxUnknownID object)
{
...
void CLxCommandNotifier::Notify(const char* name, int code)
{
    std::string         sn  = name;
    ...
    for (auto sit = set.begin(); sit != set.end(); sit++)
    {
        ...
        for (auto mit = note->pv->l_client.begin(); mit != note->pv->l_client.end(); mit++)
...
CLxNotifierHost::~CLxNotifierHost()
{
    for (auto sit = pv->clients.begin(); sit != pv->clients.end(); sit++)
...
void CLxNotifierHost::add(std::string name, std::string args)
{
    add(name.c_str(), args.c_str());
}

void CLxNotifierHost::add(const char* name, const char* args)
{
    CLxUser_NotifySysService srv;
    CLxUser_Notifier         note;
    LXtObjectID              obj;
...
void CLxNotifierHost::set_arg(CLxDynamicArguments& dyna, unsigned index)
{
    LXtObjectID         uiObj;
    ...
    lx_err::check(ui.take(uiObj));
    lx_err::check(ui.NotifierCount(&n));
    for (int i = 0; i < n; i++)
    {
        const char *name = nullptr, *args = nullptr;

        lx_err::check(ui.NotifierByIndex(i, &name, &args));
        if (name)
            add(name, args);
    }
}

void CLxNotifierHost::add_client(ILxUnknownID client)
{
...
void CLxNotifierHost::rem_client(ILxUnknownID client)
{
    if (pv->clients.find(client) == pv->clients.end())
        return;
 ...
 */
CLxOpenCommandBlock::CLxOpenCommandBlock(const char* name, unsigned flags)
{
    lx_err::check(cmd_S.BlockBegin(name, flags));
}

CLxOpenCommandBlock::~CLxOpenCommandBlock()
{
    lx_err::check(cmd_S.BlockEnd());
}
...
void CLxStaticAnalysisTest::SplitCategories(CLxUser_ValueArray va)
{
    ...
    for (auto catIter = allCategories.begin(); catIter != allCategories.end(); ++catIter)
    {
...
bool CLxStaticAnalysisTest::GetMessageTableEntry(const char* inString, const char** outMsg)
{
    ...
    srvMsg.RawText(nullptr, inString, outMsg);
    return false;
}
...
void CLxStaticAnalysisTest::RunTests(CLxUser_ValueArray vaArray)
{
    ...
    srvCmd.QueryArgString(spawnedCmd, 0, "?", va, nullptr, false);
    if (va.test())
    {
    ...
    {
        for (auto userTypeIter = allUserTypes.begin(); userTypeIter != allUserTypes.end(); ++userTypeIter)
        {
    ...
    if (useSelected < 2)
    {
        ...
        for (unsigned int itemiter = 0; itemiter < allItemCount; itemiter++)
        {
            ...
            if (useSelected == 1)
            {
                LXtID4                        _itemType = srvSel.LookupType(LXsSELTYP_ITEM);
                ...
                if ((srvSel.Test(_itemType, pkt)) == LXe_TRUE)
                {
                    bDoTest = true;
                }
                else
...
void CLxStaticAnalysisTest::sa_Select(CLxUser_Item item, std::string mode /*=""*/)
{
    ...
    srvCmd.ExecuteArgString(LXiCMD_EXEC_WITH_PARENTS_FLAGS, LXiCTAG_NULL, cmd.c_str());
}

// tags to define this is a static analysis test
LXtTagInfoDesc CLxStaticAnalysisTest::descInfo[] = { { "StaticAnalysisTest", "" }, { nullptr } };
