 ...
 */
class impl_Options_Object : public CLxImpl_StreamIO
{
public:
    ...
    static CLxPolymorph<impl_Options_Object>* factory()
    {
...
public:
    ...
    std::string              cmd_name;
    std::vector<const char*> cls_list;
    ...
    CLxLoaderOptions* alloc_options(void** ppvObj)
    {
 ...
 */
class impl_Loader_Server : public CLxImpl_Loader, public CLxImpl_TagDescription
{
public:
    class pv_Meta_Loader* mpv;
    CLxLoader*            load;

    impl_Loader_Server(pv_Meta_Loader* init = nullptr)
    {
        mpv  = init;
        load = nullptr;
    }
    ...
    LXxO_Loader_Recognize  // (const char *filename, ILxUnknownID loadInfo)
    {
        ...
        lx_err::check(load->pv->info.set(loadInfo));
        ...
        lx_err::check(load->pv->info.SetFlags(flags));
        return LXe_OK;
    }
 ...
 * the same time.
 */
const char* CLxLoader::get_class()
{
    int n;

    n = static_cast<int>(pv->mpv->cls_list.size());
    if (n == 1)
        return pv->mpv->cls_list[0];

    CLxUser_GUIDService guid_S;
    const LXtGUID *     guid{}, *gres{};
    unsigned            pri, best = std::numeric_limits<unsigned>::max();
    int                 i, ires{};
    ...
    lx_err::check(pv->info.SetClass(gres));
    return pv->mpv->cls_list[ires];
}

void CLxLoader::set_opaque(bool opaque)
{
    pv->opaque = opaque;
}

void CLxLoader::set_format(const char* fmtName)
{
    lx_err::check(pv->info.SetFormat(fmtName));
}

CLxUser_LoaderInfo& CLxLoader::loader_info()
{
    return pv->info;
}

CLxUser_Monitor& CLxLoader::monitor()
{
    return pv->mon;
}
 ...
 */
CLxMeta_Loader_Core::CLxMeta_Loader_Core(const char* srvName) : CLxMetaServer(srvName)
{
...
bool CLxMeta_Loader_Core::pre_init()
{
    CLxMeta* cmd;
...
void* CLxMeta_Loader_Core::alloc()
{
    add_tag(LXsLOD_CLASSLIST, pv->class_list().c_str());

    return reinterpret_cast<void*>(new CLxMetagenPolymorph<impl_Loader_Server, pv_Meta_Loader>(pv));
}

CLxLoaderOptions* CLxMeta_Loader_Core::access_opt(bool create)
{
...
void CLxMeta_Loader_Core::set_file_pattern(const char* pattern)
{
    add_tag(LXsLOD_DOSPATTERN, pattern);
}

void CLxMeta_Loader_Core::set_threadsafe()
{
    add_tag(LXsLOD_THREADSAFE, "Yes");
}

void CLxMeta_Loader_Core::set_class(const char* clsName)
{
 ...
 */
class impl_Saver_Server : public CLxImpl_Saver, public CLxImpl_TagDescription
{
public:
    /*
    ...
    impl_Saver_Server(pv_Meta_Saver* init = nullptr)
    {
        mpv = init;
    }
 ...
 * The metaclass holds the global settings.
 */
CLxMeta_Saver_Core::CLxMeta_Saver_Core(const char* srvName) : CLxMetaServer(srvName)
{
...
void* CLxMeta_Saver_Core::alloc()
{
    return reinterpret_cast<void*>(new CLxMetagenPolymorph<impl_Saver_Server, pv_Meta_Saver>(pv));
}

void CLxMeta_Saver_Core::set_class(const char* clsName)
{
    add_tag(LXsSAV_OUTCLASS, clsName);
}

void CLxMeta_Saver_Core::set_file_extension(const char* pattern)
{
    add_tag(LXsSAV_DOSTYPE, pattern);
}

void CLxMeta_Saver_Core::set_overwrite()
{
    add_tag(LXsSAV_OVERWRITE, "1");
}

void CLxMeta_Saver_Core::set_save_as()
{
