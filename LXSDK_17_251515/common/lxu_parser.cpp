 ...
 */
void CLxStringParser::PullWhite()
{
    while (IsBlank(sp_peek()))
        sp_advance();
}

void CLxStringParser::PullNonWhite()
{
    while (!Done() && !IsBlank(sp_peek()))
        sp_advance();
}

void CLxStringParser::PullNonWhite(std::string& dest)
{
    dest = "";
    while (!Done() && !IsBlank(sp_peek()))
    {
 ...
 */
void CLxStringParser::PullDown(std::string& dest, const char* mask)
{
    dest = "";
...
bool CLxStringParser::PullNum(float* value)
{
    double d;
...
bool CLxStringParser::PullNum(double* value)
{
    std::string buf;
...
bool CLxStringParser::PullNum(unsigned* value)
{
    std::string buf;
...
bool CLxStringParser::PullNum(int* value)
{
    int      sign;
    unsigned uval;
...
bool CLxStringParser::PullExpected(char c)
{
    PullWhite();
    if (sp_peek() != c)
        return false;
 ...
 */
bool CLxStringParser::IsBlank(char c)
{
    return (c == ' ' || c == '\t' || c == '\n');
}

bool CLxStringParser::IsDigit(char c)
{
    return (c >= '0' && c <= '9');
}

bool CLxStringParser::Matches(char c, const char* mask)
{
...
char CLxStringParser::Upper(char c)
{
    if (c >= 'a' && c <= 'z')
        return c - 'a' + 'A';
    else
...
char CLxStringParser::Lower(char c)
{
    if (c >= 'A' && c <= 'Z')
        return c - 'A' + 'a';
    else
...
public:
    ...
    std::string     filename;     // filename for reading
...
bool pv_LineParser::Open(const char* name)
{
    filename = name;
 ...
 */
void pv_LineParser::ReadLine(std::string& dest)
{
    int c, c1;
...
bool CLxLineParser::fp_Open(const char* filename)
{
    fp_Cleanup();
...
void CLxLineParser::fp_SetMonitor(LXtObjectID monObj)
{
    pv->mon.set(monObj);
    pv->InitMon();
}

bool CLxLineParser::fp_HasError()
{
    return LXx_FAIL(pv->error);
}

LxResult CLxLineParser::fp_ErrorCode()
{
    return pv->error;
}

void CLxLineParser::fp_Cleanup()
{
    pv->Close();
    cur_pos = nullptr;
}

void CLxLineParser::set_line(const char* buf)
{
...
bool CLxLineParser::lp_ReadLine(bool force)
{
    LxResult     rc;
    unsigned int len;
    bool         blank;
...
bool CLxBinaryParser::fp_Open(const char* filename)
{
    fp_Cleanup();
...
void CLxBinaryParser::fp_SetMonitor(LXtObjectID monObj)
{
    pv->mon.set(monObj);
    pv->InitMon();
}

bool CLxBinaryParser::fp_HasError()
{
    return LXx_FAIL(pv->error);
}

LxResult CLxBinaryParser::fp_ErrorCode()
{
    return pv->error;
}

void CLxBinaryParser::fp_Cleanup()
{
    pv->error    = LXe_FALSE;
    pv->filename = "";
    pv->mon.clear();
...
bool CLxBinaryParser::PullBlock(void* buf, size_t size)
{
    bool pulled(false);

    if (buf)
    {
        size_t bytesRead = fread(buf, 1, size, pv->file);
...
size_t CLxBinaryParser::StringLen()
{
    size_t length(0);
...
bool CLxBinaryParser::PullString(char* text, size_t maxSize)
{
    char* buf = text;
...
bool CLxBinaryParser::PullNum(float* value)
{
    size_t bytesRead = fread(value, 1, sizeof(float), pv->file);

    return (bytesRead == sizeof(float));
}

bool CLxBinaryParser::PullNum(double* value)
{
    size_t bytesRead = fread(value, 1, sizeof(double), pv->file);

    return (bytesRead == sizeof(double));
}

bool CLxBinaryParser::PullNum(unsigned* value)
{
    size_t bytesRead = fread(value, 1, sizeof(unsigned), pv->file);

    return (bytesRead == sizeof(unsigned));
}

bool CLxBinaryParser::PullNum(int* value)
{
    size_t bytesRead = fread(value, 1, sizeof(int), pv->file);
...
static const char* Pull_White(const char* s)
{
    if (!s)
        return nullptr;
...
static const char* Pull_RawInt(const char* p, int* val)
{
    const char* s = p;
    int         i = 0;
...
static const char* Pull_UDouble(const char* p, double* val)
{
    const char* s;
    double      v, f;
    int         k;
...
static const char* Pull_Double(const char* p, double* d)
{
    double val;
    int    sgn;
    ...
    if ((p = Pull_UDouble(p, &val)) != nullptr)
        *d = sgn * val;

    return p;
}

bool lx::Decode(const char* str, double* val)
{
    return (Pull_Double(str, val) != nullptr);
}

bool lx::Decode(const char* str, float* val)
{
    double d;
...
bool lx::Decode(std::string& str, double* val)
{
    return lx::Decode(str.c_str(), val);
}

bool lx::Decode(std::string& str, float* val)
{
    return lx::Decode(str.c_str(), val);
}
