...
static void AxisGetNormals(int axis, int* ax, int* ay)
{
...
static void MatrixIdent(Matrix m)
{
...
int GoodTriangle(const TestAxis* tax, std::vector<int>& nl, std::vector<PosVec>& pl, int n, int ori, int n1, int n2, int n3)
{
    ...
    for (i = 0; i < n; i++)
    {
        ...
        if (TestTriCorner(tax, p2, p1, p, ori) || TestTriCorner(tax, p1, p3, p, ori) || TestTriCorner(tax, p3, p2, p, ori))
        {
...
int Face_Intersect(const TestAxis* tax, int nvert, std::vector<int>& nl, std::vector<PosVec>& pl, Vector cent, double angle, int* ix, double* tx)
{
    ...
    for (i = 0; i < nvert; i++)
    {
        ...
        x3 = fv[tax->ix];
        y3 = fv[tax->iy];
        s  = ((x3 - x2) * (y2 - y0) - (y3 - y2) * (x2 - x0)) / ((x3 - x2) * (y1 - y0) - (y3 - y2) * (x1 - x0));
        t  = ((x1 - x0) * (y2 - y0) - (y1 - y0) * (x2 - x0)) / ((x3 - x2) * (y1 - y0) - (y3 - y2) * (x1 - x0));
        x2 = x3;
        y2 = y3;
        if (t < 0.0 || t > 1.0)
            continue;
...
static float FRandom(void)
{
    ...
    */
    ...
    return (rand_seed * (1.f / (float) MRAND));
}
...
int EdgeIntersectTolByPos(const TestAxis* tax, const Vector v0, const Vector v1, const Vector p0, const Vector p1, double tol, double* frac)
{
    ...
    if (det == 0.0)
    {
        ...
        #ifndef NDEBUG
        if (p0 == p1)
        qDebug ("EdgeIntersectTol DET is ZERO: EI_NONE\n");
        #endif
        */
        return EI_NONE;
    }
...
{
    triBuf[triBufInd++] = static_cast<unsigned short>(n0);
    triBuf[triBufInd++] = static_cast<unsigned short>(n1);
    triBuf[triBufInd++] = static_cast<unsigned short>(n2);
}

void Face_VertexList(std::vector<unsigned short>& triBuf,
...
void TriangulateLargeFacePolygon(std::vector<unsigned short>& triBuf,
...
/*
        ...
        possible failures.
*/
void TriangulateGeneralFacePolygon(std::vector<unsigned short>& triBuf,
                                   ...
                                   int          reverse)
{
    ...
    while (n > 3)
    {
        ...
        triBuf[k++] = static_cast<unsigned short>(n2);
        if (reverse)
        {
            triBuf[k++] = static_cast<unsigned short>(n1);
            triBuf[k++] = static_cast<unsigned short>(n3);
        }
        else
        {
            triBuf[k++] = static_cast<unsigned short>(n3);
    ...
    triBuf[k++] = static_cast<unsigned short>(nl[1]);
    if (reverse)
    {
        triBuf[k++] = static_cast<unsigned short>(nl[0]);
        triBuf[k++] = static_cast<unsigned short>(nl[2]);
    }
    else
    {
        triBuf[k++] = static_cast<unsigned short>(nl[2]);
...
std::vector<lx::GeoTriangle> lx::TriangulateFace(CLxUser_Polygon& poly, CLxUser_Point& point)
{
    LXtVector                    polyNormal;
    int                          polyAxis;
    std::vector<PosVec>          points;
    std::vector<lx::GeoTriangle> ret;
    unsigned int                 vertsNo;
    LXtPointID                   vID;
    int                          ori, reverse;
    int                          i, k;
