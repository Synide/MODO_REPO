 ...
 */
CLxAnySelection::CLxAnySelection(const char* selName)
{
    sel_ID = srv_sel.LookupType(selName);
}

void CLxAnySelection::Drop()
{
    srv_sel.Drop(sel_ID);
}

void CLxAnySelection::Clear()
{
...
bool CLxVertexSelection::AnyDirect()
{
    return srv_sel.Count(sel_ID) != 0;
}

bool CLxVertexSelection::AnyIndirect()
{
    LXtID4 cur;
...
void CLxVertexSelection::GetDirectList(MeshList& list)
{
    CLxUser_Item  item;
    MeshEntry     entry;
    LXtScanInfoID scan;
    ...
    scan = nullptr;
    while ((scan = srv_sel.ScanLoop(scan, sel_ID, &pkt)) != nullptr)
    {
...
bool CLxVertexMapSelection::First(std::string& name)
{
    LXtScanInfoID scan;
    LXtID4        type;
    const char*   str;
...
void CLxVertexMapSelection::GetList(MapList& list)
{
    LXtScanInfoID scan;
    MapInfo       info;
    const char*   str;
...
bool CLxItemSelection::GetFirst(CLxUser_Item& item)
{
    LXtScanInfoID scan;
    void*         pkt;
...
void CLxItemSelection::GetList(ItemList& list)
{
    CLxUser_Item  item;
    LXtScanInfoID scan;
    void*         pkt;
...
void CLxItemSelection::GetSet(ItemSet& set)
{
    CLxUser_Item  item;
    LXtScanInfoID scan;
    void*         pkt;
...
void CLxItemSelection::LoopInit()
{
    GetList(tmp_list);
    tmp_idx = 0;
}

void CLxItemSelection::LoopStop()
{
    tmp_list.clear();
    tmp_idx = 0;
}

bool CLxItemSelection::LoopNext(CLxUser_Item& item)
{
...
void CLxItemSelection::Drop()
{
    srv_sel.Drop(sel_ID);
}

void CLxItemSelection::Clear()
{
    srv_sel.Clear(sel_ID);
}

void CLxItemSelection::Select(CLxUser_Item& item)
{
    srv_sel.Select(sel_ID, pkt_trans.Packet(item));
}

void CLxItemSelection::Deselect(CLxUser_Item& item)
{
 ...
 */
bool CLxItemSelectionType::Include(CLxUser_Item& item)
{
 ...
 */
void CLxItemCurrentTypes::invalidate()
{
...
void CLxItemCurrentTypes::SetDefault()
{
...
void CLxItemCurrentTypes::SetString(const char* mask)
{
    CLxUser_SceneService scene_S;
...
bool CLxItemCurrentTypes::Test(LXtItemType type)
{
    int k, i;
...
bool CLxSceneSelection::Get(CLxUser_Scene& scene)
{
    void* pkt;
...
bool CLxSceneSelection::Get(CLxUser_ChannelRead& evalChan)
{
    CLxUser_Scene scene;
...
bool CLxSceneSelection::Get(CLxUser_ChannelRead& actChan, const char* action)
{
    CLxUser_Scene scene;
 ...
 */
CLxChannelSelection::CLxChannelSelection() : CLxAnySelection(LXsSELTYP_CHANNEL)
{
    pkt_trans.autoInit();
}

bool CLxChannelSelection::GetFirst(CLxUser_Item& item, unsigned& index)
{
    LXtScanInfoID scan;
    void*         pkt;
...
void CLxChannelSelection::GetList(ChanList& list)
{
    Chan          chan;
    LXtScanInfoID scan;
    void*         pkt;
...
void CLxChannelSelection::GetSet(ChanSet& set)
{
    Chan          chan;
    LXtScanInfoID scan;
    void*         pkt;
...
void CLxChannelSelection::LoopInit()
{
    GetList(tmp_list);
    tmp_idx = 0;
}

void CLxChannelSelection::LoopStop()
{
    tmp_list.clear();
    tmp_idx = 0;
}

bool CLxChannelSelection::LoopNext(CLxUser_Item& item, unsigned& index)
{
    if (tmp_idx < tmp_list.size())
    {
...
void CLxChannelSelection::Select(CLxUser_Item& item, unsigned index)
{
    srv_sel.Select(sel_ID, pkt_trans.Packet(item, index));
}

void CLxChannelSelection::Deselect(CLxUser_Item& item, unsigned index)
{
    srv_sel.Deselect(sel_ID, pkt_trans.Packet(item, index));
}

void CLxChannelSelection::Select(CLxUser_Item& item, const char* name)
{
    Select(item, item.ChannelIndex(name));
}

void CLxChannelSelection::Deselect(CLxUser_Item& item, const char* name)
{
...
public:
    ...
    const char*                 srv_name;
    const char*                 m_table;
    CLxMeta_SelectionType_Core* m_core;
    CLxUser_SelectionService    sel_S;
    LXtID4                      id_code;
    bool                        is_undo;
    ...
    pv_Meta_SelectionType()
    {
        ...
        m_table  = nullptr;
        m_core   = nullptr;
        id_code  = std::numeric_limits<LXtID4>::max();
        is_undo  = false;
    }
 ...
 */
class impl_SelectionType_Server : public CLxImpl_SelectionType, public CLxImpl_TagDescription
{
public:
     ...
     */
    impl_SelectionType_Server(pv_Meta_SelectionType* init = nullptr)
    {
        mpv  = init;
    ...
    {
        return static_cast<unsigned>(inst->size());
    }
 ...
 */
CLxMeta_SelectionType_Core::CLxMeta_SelectionType_Core(const char* srvName, const char* id) : CLxMetaServer(srvName)
{
...
void* CLxMeta_SelectionType_Core::alloc()
{
    return reinterpret_cast<void*>(new CLxMetagenPolymorph<impl_SelectionType_Server, pv_Meta_SelectionType>(pv));
}

void CLxMeta_SelectionType_Core::set_undoable(bool isUndoable)
{
    pv->is_undo = isUndoable;
}

LXtID4 CLxMeta_SelectionType_Core::get_ID4()
{
    return pv->id_code;
}

void CLxMeta_SelectionType_Core::v_select(void* pkt, bool add)
{
    if (!add)
        clear();
...
void CLxMeta_SelectionType_Core::v_deselect(void* pkt)
{
    pv->sel_S.set();
    lx_err::check(pv->sel_S.Deselect(pv->id_code, pkt));
}

void CLxMeta_SelectionType_Core::clear()
{
    pv->sel_S.set();
    lx_err::check(pv->sel_S.Clear(pv->id_code));
}

void CLxMeta_SelectionType_Core::drop()
{
    pv->sel_S.set();
