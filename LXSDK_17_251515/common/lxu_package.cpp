 ...
 * interface guids.
 */
LxResult CLxDefaultPackage::pkg_Attach(void** ppvObj)
{
    Spawner sp;
...
LxResult CLxDefaultPackage::pkg_TestInterface(const LXtGUID* guid)
{
    Spawner sp;
 ...
 */
class meta_PackageInstance : public CLxImpl_PackageInstance
{
public:
    CLxPackage* m_pkg;
 ...
 */
class meta_PackageInstancePolymorph : public CLxPolymorph<meta_PackageInstance>
{
public:
    class pv_Meta_Package* pv;

    meta_PackageInstancePolymorph(class pv_Meta_Package* ptr)
    {
    ...
    bool TestInterface(const LXtGUID* iid) override
    {
    ...
    ILxUnknownID IsA(ILxUnknownID /*src*/, const LXtGUID* iid) override
    {
 ...
 */
class meta_PackageServer : public CLxImpl_Package, public CLxImpl_TagDescription
{
public:
    class pv_Meta_Package* pv;
 ...
 */
class meta_PackagePolymorph : public CLxPolymorph<meta_PackageServer>
{
public:
    class pv_Meta_Package* pv;

    meta_PackagePolymorph(class pv_Meta_Package* ptr)
    {
 ...
 */
CLxMeta_Package_Core::CLxMeta_Package_Core(const char* srvName) : CLxMetaServer(srvName)
{
...
bool CLxMeta_Package_Core::pre_init()
{
    CLxMeta* m;
...
void* CLxMeta_Package_Core::alloc()
{
...
void CLxMeta_Package_Core::set_supertype(const char* typeName)
{
 ...
 */
class meta_ChannelUI : public CLxImpl_ChannelUI
{
public:
 ...
 */
class meta_ChannelUIPolymorph : public CLxPolymorph<meta_ChannelUI>
{
public:
    pv_Meta_ChannelUI* pv;

    meta_ChannelUIPolymorph(pv_Meta_ChannelUI* init)
    {
...
void* CLxMeta_ChannelUI_Core::alloc()
{
    CLxMeta* m;
...
public:
    ...
    pv_Meta_ViewItem3D(CLxMeta_ViewItem3D_Core* init)
    {
        core    = init;
        b_world = false;
        b_bg    = false;
...
class meta_ViewItem3D : public CLxImpl_ViewItem3D
{
public:
...
class meta_ViewItem3DPolymorph : public CLxMetaPackagePolymorph<meta_ViewItem3D>
{
public:
    pv_Meta_ViewItem3D* pv;

    meta_ViewItem3DPolymorph(pv_Meta_ViewItem3D* init)
    {
        this->AddInterface(new CLxIfc_ViewItem3D<meta_ViewItem3D>);
        pv = init;
    }

    void inherit(meta_ViewItem3D* that, CLxPackage* pkg) override
    {
...
void CLxMeta_ViewItem3D_Core::set_world_space(bool world)
{
    pv->b_world = world;
}

void CLxMeta_ViewItem3D_Core::enable_background()
{
    pv->b_bg = true;
}

void CLxMeta_ViewItem3D_Core::enable_test()
{
    pv->b_test = true;
}

void* CLxMeta_ViewItem3D_Core::alloc()
{
...
void CLxInstanceAssets::add_asset(const char* ident, const char* fileType, const char* category, bool isSeq)
{
...
public:
    ...
    pv_Meta_InstanceAssets(CLxMeta_InstanceAssets_Core* init)
    {
        core = init;
    }
};
 ...
 */
class meta_InstanceAssets : public CLxImpl_InstanceAssets
{
public:
    ...
    LXxO_InstanceAssets_Count
    {
        ...
        *count = static_cast<unsigned>(i_ass->pv->ass_list.size());
        return LXe_OK;
    }
...
class meta_InstanceAssetsPolymorph : public CLxMetaPackagePolymorph<meta_InstanceAssets>
{
public:
    pv_Meta_InstanceAssets* mpv;

    meta_InstanceAssetsPolymorph(pv_Meta_InstanceAssets* init)
    {
        this->AddInterface(new CLxIfc_InstanceAssets<meta_InstanceAssets>);
        mpv = init;
    }

    void inherit(meta_InstanceAssets* that, CLxPackage* pkg) override
    {
        that->m_item.set(pkg->m_item);
        that->mpv   = mpv;
...
void* CLxMeta_InstanceAssets_Core::alloc()
{
