...
public:
    ...
    bool operator==(const CChannelInfo& rhs) const
    {
        return (item == rhs.item && index == rhs.index && type == rhs.type);
    }
};

class CChannelList : public std::vector<CChannelInfo>
{
public:
    void add(CLxUser_Item item, unsigned index, unsigned type)
    {
        CChannelInfo proto;
...
public:
    ...
    CLxMeta*                       alt_eval;
    CLxPolymorph<pv_EvalModifier>* mod_spawn;
    ...
    void add_list_string(std::set<std::string>& list, const char* name)
    {
        std::string val(name);
        list.insert(val);
    }
     ...
     */
    void set_meta(CLxMetaServer* meta)
    {
        if (self_srv)
            return;
...
CLxMeta_EvalModifier_Core::CLxMeta_EvalModifier_Core(const char* srvName) : CLxMetaServer(srvName)
{
 ...
 */
void CLxMeta_EvalModifier_Core::set_itemtype(const char* typeName)
{
    pv->set_type(typeName);
}

void CLxMeta_EvalModifier_Core::add_dependent_type(const char* typeName)
{
    pv->add_list_string(pv->dep_types, typeName);
    pv->has_custom = true;
}

void CLxMeta_EvalModifier_Core::add_dependent_graph(const char* graph)
{
    pv->add_list_string(pv->dep_graphs, graph);
    pv->has_custom = true;
}

void CLxMeta_EvalModifier_Core::get_all_items()
{
    pv->all_types = true;
}

void CLxMeta_EvalModifier_Core::invalidate(CLxUser_Scene& scene, bool reset)
{
    if (reset)
...
void CLxMeta_EvalModifier_Core::invalidate(CLxUser_Item& item, bool reset)
{
    CLxUser_Scene us;
...
void CLxMeta_EvalModifier_Core::invalidate(ILxUnknownID scene, bool reset)
{
 ...
 * evaluation methods.
 */
class pv_EvalModifier : public CLxImpl_Modifier
{
public:
    ...
    void init(CLxMeta_EvalModifier_Core* core, ILxUnknownID item, unsigned index)
    {
        m_core      = core;
        m_cpv       = core->pv;
        m_com       = core->new_inst();
        ...
        lx_err::check(m_item.set(item));
        m_index = index;
    }

    void set_eval(ILxUnknownID eval)
    {
        lx_err::check(m_eval.set(eval));
        lx_err::check(m_attr.set(eval));
    }

    void bind_chans(bool isReal)
    {
    ...
    void link_chans()
    {
        ...
        n = static_cast<int>(cust_chan.size());
        for (i = 0; i < n; i++)
        {
            ci = &cust_chan[i];
            lx_err::check(m_eval.AddChannel(ci->item, ci->index, ci->type, &index));
            if (i == 0)
                base_cust = index;
        }
    }

    void bind_and_link(ILxUnknownID eval)
    {
        set_eval(eval);
        bind_chans(true);
        link_chans();
...
class polymorph_Modifier : public CLxExtendedPolymorph<pv_EvalModifier>
{
public:
    polymorph_Modifier(CLxMetaInterfaces* ifcs) : CLxExtendedPolymorph<pv_EvalModifier>(ifcs)
    {
        AddInterface(new CLxIfc_Modifier<pv_EvalModifier>);
    }

    void* RefObj(void* obj) override
    {
...
static CLxPolymorph<pv_EvalModifier>* alloc_ModSpawn(CLxMetaInterfaces* ifcs)
{
 ...
 */
class metaServer_EvalModifier : public CLxImpl_EvalModifier, public CLxImpl_TagDescription, public CLxObject
{
public:
...
class metaPolymorph_EvalModifier : public CLxPolymorph<metaServer_EvalModifier>
{
public:
...
void* CLxMeta_EvalModifier_Core::alloc()
{
...
public:
    ...
    pv_EvalModifier_Server(CLxMeta_EvalModifier_Core* cp)
    {
        core      = cp;
        com       = cp->new_inst();
        com->psrv = this;
     ...
     */
    static void get_list(std::set<std::string>& list, std::string& str)
    {
    ...
    void reset_item()
    {
        ...
        lx_err::check(iter_scene.ItemCount(iter_type, &iter_num));
        iter_cur = 0;
    }

    LXtObjectID next_item(unsigned* index)
    {
        if (iter_cur >= iter_num)
            return nullptr;

        CLxUser_Item item;
        lx_err::check(iter_scene.ItemByIndex(iter_type, iter_cur++, item));
        index[0] = 0;
        return item.m_loc;
    }
    ...
    void reset_chan()
    {
        lx_err::check(iter_scene.ItemCount(-1, &iter_num));
        iter_cur = 0;
        pending_nodes.clear();
    }

    LXtObjectID next_chan(unsigned* index)
    {
        while (1)
        {
            ...
            lx_err::check(iter_scene.ItemByIndex(-1, iter_cur++, cur_item));
...
void metaServer_EvalModifier::init(CLxMeta_EvalModifier_Core* core)
{
...
LxResult metaServer_EvalModifier::eval_Reset(ILxUnknownID scene)
{
...
LXtObjectID metaServer_EvalModifier::eval_Next(unsigned* index)
{
    if (pv->include_by_type())
...
LxResult metaServer_EvalModifier::eval_Alloc(ILxUnknownID item, unsigned index, ILxUnknownID eval, void** ppvObj)
{
    pv_EvalModifier* mod;
...
unsigned metaServer_EvalModifier::tag_Count()
{
    pv->init_tags();
    return pv->tag_list->count();
}

LxResult metaServer_EvalModifier::tag_Describe(unsigned index, LXtTagInfoDesc* desc)
{
    pv->init_tags();
...
void CLxEvalModifier::mod_add_node(unsigned index)
{
    CChannelInfo info{};
...
void CLxEvalModifier::mod_read_attr(void* ptr)
{
    pmod->m_cpv->attr_desc->eval_read(pmod->m_attr, pmod->base_desc, ptr);
}

void CLxEvalModifier::mod_add_chan(CLxUser_Item& item, const char* channel, unsigned type)
{
    unsigned index;

    lx_err::check(item.ChannelLookup(channel, &index));
    mod_add_chan(item, index, type);
}

void CLxEvalModifier::mod_add_chan(CLxUser_Item& item, unsigned channel, unsigned type)
{
    pmod->cust_chan.add(item, channel, type);
    pmod->m_cpv->has_custom = true;
}

CLxUser_Item* CLxEvalModifier::mod_item()
{
    return &pmod->m_item;
}

unsigned CLxEvalModifier::mod_index()
{
    return pmod->m_index;
}

CLxUser_Evaluation* CLxEvalModifier::mod_eval()
{
    return &pmod->m_eval;
}

CLxUser_Attributes* CLxEvalModifier::mod_attr()
{
    return &pmod->m_attr;
}

void CLxEvalModifier::mod_result(LxResult rc)
{
    pmod->res_value = rc;
}

unsigned CLxEvalModifier::mod_cust_index(unsigned offset)
{
    return pmod->base_cust + offset;
}

void CLxEvalModifier::mod_cust_val(unsigned offset, int* result)
{
    lx_err::check(pmod->m_attr.GetInt(mod_cust_index(offset), result));
}

void CLxEvalModifier::mod_cust_val(unsigned offset, bool* result)
{
    int k;

    lx_err::check(pmod->m_attr.GetInt(mod_cust_index(offset), &k));
    result[0] = !!k;
}

void CLxEvalModifier::mod_cust_val(unsigned offset, double* result)
{
    lx_err::check(pmod->m_attr.GetFlt(mod_cust_index(offset), result));
}

void CLxEvalModifier::mod_cust_val(unsigned offset, LXtMatrix4& m4)
{
    CLxUser_Matrix mat;

    lx_err::check(pmod->m_attr.Value(mod_cust_index(offset), mat, 0));
    lx_err::check(mat.Get4(m4));
}

void CLxEvalModifier::mod_cust_write(unsigned offset, bool value)
{
    lx_err::check(pmod->m_attr.SetInt(mod_cust_index(offset), !!value));
}

void CLxEvalModifier::mod_cust_write(unsigned offset, int value)
{
    lx_err::check(pmod->m_attr.SetInt(mod_cust_index(offset), value));
}

void CLxEvalModifier::mod_cust_write(unsigned offset, double value)
{
    lx_err::check(pmod->m_attr.SetFlt(mod_cust_index(offset), value));
}

void CLxEvalModifier::mod_cust_write(unsigned offset, const LXtMatrix4& m4)
{
    CLxUser_Matrix mat;

    lx_err::check(pmod->m_attr.Value(mod_cust_index(offset), mat, 1));
    lx_err::check(mat.Set4(m4));
}
 ...
 */
class impl_Simulation : public CLxImpl_SimulationModifier
{
public:
    CLxSimulation* sim;
...
class polymorph_Simulation : public CLxPolymorph<impl_Simulation>
{
public:
    ...
    void InitObj(void* sim, void* mod) override
    {
...
class meta_Simulation : public CLxMeta
{
public:
    meta_Simulation()
    {
...
void CLxMeta_EvalModifier_Core::set_simulation()
{
 ...
 */
unsigned CLxItemModifierServer::tag_Count()
{
    if (!l_tags.size())
    {
...
LxResult CLxItemModifierServer::tag_Describe(unsigned index, LXtTagInfoDesc* desc)
{
 ...
 */
LxResult CLxItemModifierServer::eval_Reset(ILxUnknownID scene)
{
...
void CLxItemModifierServer::UpdateList(CLxUser_Scene& scene)
{
    CLxUser_Item item;
    unsigned     i, n;
...
void CLxItemModifierServer::add_entry(CLxUser_Item& item, unsigned index)
{
    l_item.push_back(item);
    l_indx.push_back(index);
}

LXtObjectID CLxItemModifierServer::eval_Next(unsigned* index)
{
    LXtObjectID obj;
 ...
 */
LxResult CLxItemModifierServer::eval_Alloc(ILxUnknownID item, unsigned index, ILxUnknownID eval, void** ppvObj)
{
 ...
 */
LxResult CLxItemModifierNode::mod_Test(ILxUnknownID item, unsigned /*index*/)
{
 ...
 */
LxResult CLxItemModifierNode::mod_Evaluate()
{
    CLxObject* cache = nullptr;
    bool       prev_eval{};

    if (has_cache)
    {
        cache     = (CLxObject*) m_eval.GetCache();
        prev_eval = (cache != nullptr);
        if (!prev_eval)
        {
            cache = p_elt->Cache();
            if (!cache)
 ...
 */
void CLxItemModifierNode::mod_Free(void* cacheRaw)
{
 ...
 */
LxResult CLxItemModifierNode::mod_Validate(ILxUnknownID item, unsigned channel, LxResult rc)
{
 ...
 */
void CLxObjectRefModifierCore::Eval(CLxUser_Evaluation& eval, CLxUser_Attributes& attr)
{
 ...
 */
void CLxRenderCamera::Camera(CLxUser_Evaluation& eval, CLxUser_Item& camera)
{
...
void CLxRenderCamera::BuildList(void)
{
    ILxUnknownID  camObj;
    CLxUser_Scene scene(sceneObj);
    CLxUser_Item  camera;
...
bool CLxRenderCamera::ListIsValid(void)
{
    ILxUnknownID  camObj{};
    CLxUser_Scene scene(sceneObj);
    CLxUser_Item  camera;
...
unsigned CLxRenderCamera::AttachIndexChan(CLxUser_Evaluation& eval)
{
    CLxUser_Scene scene(sceneObj);
...
int CLxRenderCamera::Camera(CLxUser_Attributes& attr)
{
    return attr.Int(this->chanIndex);
}

size_t CLxRenderCamera::size(void)
{
    return cameraList.size();
}

ILxUnknownID& CLxRenderCamera::operator[](const int index)
{
