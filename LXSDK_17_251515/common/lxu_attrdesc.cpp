...
public:
    ...
    public:
         ...
         * by index.
         */
        void vec_channel(std::string& full, int i)
        {
            full = name;
            full += ".";
            full += vec_type[i];
         ...
         */
        template <class T>
        void p_set(void* ptr, const T& value, int index = 0)
        {
        ...
        void p_set(void* ptr, ILxUnknownID value)
        {
            char*               base = reinterpret_cast<char*>(ptr);
        ...
        template <class T>
        void p_get(void* ptr, T& value, int index = 0)
        {
            char*    base = reinterpret_cast<char*>(ptr);
        ...
        template <class T>
        int p_comp(void* p1, void* p2, int index = 0)
        {
            T t1, t2;
        ...
        void range_hints(CLxUser_UIHints& hint)
        {
            if (r_min.exist)
            {
                if (r_min.isint)
    ...
    void add(const char* name, const char* type)
    {
        ...
        cur->vec_type   = nullptr;
        cur->vec_N      = 0;
        cur->text_hint  = nullptr;
        cur->arg_flag   = 0;
     ...
     * for this).
     */
    Attr* lookup(unsigned index, LxResult err = LXe_OUTOFBOUNDS)
    {
        Attr* at;
     ...
     */
    Attr* lookup(const char* name, LxResult err = LXe_OUTOFBOUNDS)
    {
        Attr*       at;
        std::string sub;
        int         len;
        ...
        for (at = list.first; at; at = at->next)
            if (at->name.compare(name) == 0 && (at->vec_type != nullptr) == isvec)
                return at;
     ...
     */
    const char* by_index(unsigned index)
    {
        Attr* at;
    ...
    unsigned by_name(const char* name)
    {
        Attr* at;
     ...
     */
    unsigned eval_attach(CLxUser_Evaluation& eval, ILxUnknownID item)
    {
        Attr*    at;
        unsigned k, r, idx;
     ...
     */
    void eval_read(CLxUser_Attributes& attr, unsigned index, void* ptr)
    {
        Attr* at;
     ...
     */
    void chan_read(CLxUser_ChannelRead& read, CLxUser_Item& item, void* ptr)
    {
        Attr* at;
     ...
     */
    void chanmod_define(CLxUser_ChannelModSetup& setup)
    {
        Attr* at;
        ...
        for (at = list.first; at; at = at->next)
        {
            ...
            else if (at->vec_type)
            {
                ...
                {
                    at->vec_channel(name, i);
                    lx_err::check(setup.AddChannel(name.c_str(), at->chmod_flags));
                }
            }
            else
            {
                lx_err::check(setup.AddChannel(at->name.c_str(), at->chmod_flags));
            }
        }
    }
     ...
     */
    void chanmod_bind(CLxUser_ChannelModSetup& setup, void* ptr)
    {
        char*               base = reinterpret_cast<char*>(ptr);
        void*               dest;
        ...
        for (at = list.first; at; at = at->next)
        {
            ...
            else if (at->vec_type)
            {
                ...
                for (int i = 0; i < at->vec_N; i++)
                {
                    at->vec_channel(name, i);
                    if (at->chmod_flags & LXfCHMOD_OUTPUT)
                        lx_err::check(setup.WriteValue(name.c_str(), pvalue[i]));
                    else
                        lx_err::check(setup.ReadValue(name.c_str(), pvalue[i]));
                }
            }
            else
            {
                pvalue = reinterpret_cast<CLxUser_Value*>(dest);
                if (at->chmod_flags & LXfCHMOD_OUTPUT)
                    lx_err::check(setup.WriteValue(at->name.c_str(), *pvalue));
                else
                    lx_err::check(setup.ReadValue(at->name.c_str(), *pvalue));
            }
        }
    }
     ...
     */
    void setup_args(CLxDynamicArguments& cmd)
    {
        Attr* at;
     ...
     */
    void read_args(CLxDynamicAttributes& cmd, void* ptr)
    {
        Attr* at;
     ...
     */
    void read_args_isset(CLxDynamicAttributes& cmd, void* ptr)
    {
        char* base = reinterpret_cast<char*>(ptr);
        Attr* at;
     ...
     */
    void dialog_init(CLxDynamicAttributes& cmd)
    {
        Attr* at;
    ...
    int struct_compare(void* p1, void* p2)
    {
        ...
        for (at = list.first; at; at = at->next)
        {
            ...
            {
                for (auto i = 0; i < at->vec_N; i++)
                {
                    if (at->p_comp<double>(p1, p2, i))
                        break;
                }
            }
     ...
     * subsequent methods.
     */
    void nodal_link(ILxUnknownID eval, ILxUnknownID item, int* idx)
    {
     ...
     * 'fixed' struct.
     */
    void nodal_read(ILxUnknownID etor, int* idx, void* dest, void* fixed)
    {
        Attr* at;
     ...
     */
    bool to_string(std::string& result, Attr* at, void* ptr)
    {
 ...
 */
void CLxAttributeDesc::add(const char* name, const char* type)
{
    pv->add(name, type);
}

void CLxAttributeDesc::vector_type(const char* vtype)
{
    pv->cur->vec_type = vtype;
    pv->cur->vec_N    = (int) strlen(vtype);
}

void CLxAttributeDesc::default_val(double value)
{
    pv->cur->def_type = pv_AttributeDesc::TYP_DOUBLE;
    pv->cur->def_f    = value;
}

void CLxAttributeDesc::default_val(int value)
{
    pv->cur->def_type = pv_AttributeDesc::TYP_INT;
    pv->cur->def_i    = value;
}

void CLxAttributeDesc::default_val(bool value)
{
    pv->cur->def_type = pv_AttributeDesc::TYP_BOOL;
    pv->cur->def_i    = (value ? 1 : 0);
}

void CLxAttributeDesc::default_val(const char* value)
{
    pv->cur->def_type = pv_AttributeDesc::TYP_STRING;
    pv->cur->def_str  = value;
}

void CLxAttributeDesc::default_val(std::string& value)
{
    pv->cur->def_type = pv_AttributeDesc::TYP_STRING;
    pv->cur->def_str  = value;
}
...
void CLxAttributeDesc::struct_offset(double* value)
{
    pv->cur->st_type   = pv_AttributeDesc::TYP_DOUBLE;
    pv->cur->st_offset = int(size_t(value));
}

void CLxAttributeDesc::struct_offset(int* value)
{
    pv->cur->st_type   = pv_AttributeDesc::TYP_INT;
    pv->cur->st_offset = int(size_t(value));
}

void CLxAttributeDesc::struct_offset(bool* value)
{
    pv->cur->st_type   = pv_AttributeDesc::TYP_BOOL;
    pv->cur->st_offset = int(size_t(value));
}

void CLxAttributeDesc::struct_offset(std::string* value)
{
...
void CLxAttributeDesc::struct_offset(LXtVector* value)
{
    struct_offset((double**) value);
}

void CLxAttributeDesc::struct_offset(CLxLocalizedObject* value)
{
    pv->cur->st_type   = pv_AttributeDesc::TYP_OBJECT;
    pv->cur->st_offset = int(size_t(value));
}

void CLxAttributeDesc::isset_offset(bool* value)
{
    pv->cur->st_hasset = true;
    pv->cur->st_isset  = int(size_t(value));
}

void CLxAttributeDesc::chanmod_chan(int flags, CLxUser_Value* value)
{
...
void CLxAttributeDesc::chanmod_chan(int flags, CLxUser_Matrix* value)
{
    pv->cur->chmod_ok    = true;
    pv->cur->chmod_flags = flags & ~LXfCHMOD_MULTILINK;
...
void CLxAttributeDesc::chanmod_chan(int flags, CLxUser_ValueArray* value)
{
    pv->cur->chmod_ok    = true;
    pv->cur->chmod_flags = flags | LXfCHMOD_MULTILINK;
...
void CLxAttributeDesc::chanmod_time(CLxUser_Value* value)
{
    pv->add("-time-", "time");
...
void CLxAttributeDesc::eval_flag(unsigned flag)
{
    pv->cur->eval_ok   = true;
    pv->cur->eval_flag = flag;
}

void CLxAttributeDesc::eval_time(double* value)
{
    pv->add("-time-", "time");
...
void CLxAttributeDesc::arg_flag(unsigned flag)
{
    pv->cur->arg_flag = flag;
}

void CLxAttributeDesc::set_hint(LXtTextValueHint* hint)
{
    pv->cur->text_hint = hint;
}

void CLxAttributeDesc::nodal_input()
{
    pv->cur->node_ok = true;
}

void CLxAttributeDesc::bool_type(int type)
{
    pv->cur->bool_type = type;
}

void CLxAttributeDesc::set_storage()
{
    pv->cur->is_storage = true;
}

void CLxAttributeDesc::chan_add_dependency(const char* channel, const char* itemtype)
{
...
void CLxAttributeDesc::chan_set_custom(CLxCustomChannelUI* ui)
{
    pv->cur->chan_ui = ui;
...
void CLxAttributeDesc::arg_set_custom(CLxCustomArgumentUI* ui)
{
    pv->cur->arg_ui = ui;
}

void CLxAttributeDesc::chan_flags(unsigned flags)
{
    pv->cur->chan_flags = flags;
    pv->chanUI          = true;
}

void CLxAttributeDesc::set_uivalue(CLxDynamicUIValue* uival)
{
    pv->cur->ui_val = uival;
    pv->chanUI      = true;
}

void CLxAttributeDesc::set_min(int val, bool clamp)
{
...
void CLxAttributeDesc::set_max(int val, bool clamp)
{
...
void CLxAttributeDesc::set_min(double val, bool clamp)
{
...
void CLxAttributeDesc::set_max(double val, bool clamp)
{
 ...
 */
unsigned CLxAttributeDesc::count()
{
    return pv->count;
}

LxResult CLxAttributeDesc::count(unsigned* intPtr)
{
    intPtr[0] = count();
    return LXe_OK;
}

const char* CLxAttributeDesc::by_index(unsigned index)
{
    return pv->by_index(index);
}

unsigned CLxAttributeDesc::by_name(const char* name)
{
    return pv->by_name(name);
}

bool CLxAttributeDesc::need_chan_ui()
{
    return pv->chanUI;
}

int CLxAttributeDesc::struct_compare(void* p1, void* p2)
{
 ...
 */
LxResult CLxAttributeDesc::setup_channels(ILxUnknownID addchan)
{
    return pv->setup_channels(addchan);
}

unsigned CLxAttributeDesc::eval_attach(CLxUser_Evaluation& eval, ILxUnknownID item)
{
    return pv->eval_attach(eval, item);
}

unsigned CLxAttributeDesc::eval_attach(ILxUnknownID evalObj, ILxUnknownID item)
{
...
void CLxAttributeDesc::eval_read(CLxUser_Attributes& attr, unsigned index, void* ptr)
{
    pv->eval_read(attr, index, ptr);
}

void CLxAttributeDesc::eval_read(ILxUnknownID attrObj, unsigned index, void* ptr)
{
...
void CLxAttributeDesc::chan_read(CLxUser_ChannelRead& read, CLxUser_Item& item, void* ptr)
{
    pv->chan_read(read, item, ptr);
}

void CLxAttributeDesc::chan_read(ILxUnknownID chanRead, ILxUnknownID itemObj, void* ptr)
{
...
LxResult CLxAttributeDesc::chanmod_define(ILxUnknownID cmod)
{
...
void CLxAttributeDesc::chanmod_bind(ILxUnknownID cmod, void* ptr)
{
    CLxUser_ChannelModSetup setup(cmod);
...
void CLxAttributeDesc::setup_args(CLxDynamicArguments& cmd)
{
    pv->setup_args(cmd);
}

void CLxAttributeDesc::read_args(CLxDynamicAttributes& cmd, void* ptr)
{
    pv->read_args(cmd, ptr);
    pv->read_args_isset(cmd, ptr);
}

void CLxAttributeDesc::read_args_isset(CLxDynamicAttributes& cmd, void* ptr)
{
    pv->read_args_isset(cmd, ptr);
}

LxResult CLxAttributeDesc::dialog_init(CLxDynamicAttributes& cmd)
{
    pv->dialog_init(cmd);
    return LXe_OK;
}

const char* CLxAttributeDesc::arg_get_type(unsigned index, CLxCommand& inst)
{
...
bool CLxAttributeDesc::arg_enabled(unsigned index, CLxCommand& inst)
{
    pv_AttributeDesc::Attr* at = pv->lookup(index);
...
void CLxAttributeDesc::arg_query(unsigned index, CLxCommand& inst)
{
    pv_AttributeDesc::Attr* at = pv->lookup(index);
...
void CLxAttributeDesc::arg_uihints(unsigned index, CLxUser_UIHints& hint, CLxCommand& inst)
{
    pv_AttributeDesc::Attr* at = pv->lookup(index);
...
CLxDynamicUIValue* CLxAttributeDesc::arg_uivalue(unsigned index, CLxCommand& inst)
{
    pv_AttributeDesc::Attr* at = pv->lookup(index);
...
LxResult CLxAttributeDesc::nodal_link(ILxUnknownID eval, ILxUnknownID item, int* idx)
{
    pv->nodal_link(eval, item, idx);
    return LXe_OK;
}

LxResult CLxAttributeDesc::nodal_driven(int* idx)
{
    return (pv->nodal_driven(idx) ? LXe_TRUE : LXe_FALSE);
}

void CLxAttributeDesc::nodal_read(ILxUnknownID etor, int* idx, void* dest, void* fixed)
{
    pv->nodal_read(etor, idx, dest, fixed);
}

LxResult CLxAttributeDesc::chan_enabled(const char* name, ILxUnknownID itemObj, ILxUnknownID chanRead, ILxUnknownID message)
{
...
int CLxAttributeDesc::chan_dep_count(const char* name)
{
    pv_AttributeDesc::Attr* at = pv->lookup(name, LXe_NOTIMPL);

    return static_cast<int>(at->chan_deps.size());
}

void CLxAttributeDesc::chan_dep_byindex(const char* name, unsigned index, const char** chan, const char** type)
{
    pv_AttributeDesc::Attr* at = pv->lookup(name, LXe_NOTIMPL);
...
LxResult CLxAttributeDesc::chan_uihints(const char* name, ILxUnknownID hobj)
{
    pv_AttributeDesc::Attr* at = pv->lookup(name, LXe_NOTIMPL);
...
LxResult CLxAttributeDesc::chan_uivalue(const char* name, void** ppvObj)
{
    pv_AttributeDesc::Attr* at    = pv->lookup(name, LXe_NOTIMPL);
    CLxDynamicUIValue*      uival = nullptr;
    bool                    own   = true;
...
unsigned int CLxAttributesFromDesc::attr_Count()
{
    return afd_desc->pv->count;
}

LxResult CLxAttributesFromDesc::attr_Name(unsigned int index, const char** name)
{
    name[0] = afd_desc->by_index(index);
    return LXe_OK;
}

LxResult CLxAttributesFromDesc::attr_Lookup(const char* name, unsigned int* index)
{
    index[0] = afd_desc->by_name(name);
    return LXe_OK;
}

LxResult CLxAttributesFromDesc::attr_Type(unsigned int index, unsigned int* type)
{
...
LxResult CLxAttributesFromDesc::attr_TypeName(unsigned int index, const char** tname)
{
    pv_AttributeDesc::Attr* at = afd_desc->pv->lookup(index);
...
const LXtTextValueHint* CLxAttributesFromDesc::attr_Hints(unsigned int index)
{
    pv_AttributeDesc::Attr* at = afd_desc->pv->lookup(index);
    return at->text_hint;
}

LxResult CLxAttributesFromDesc::attr_Value(unsigned int /*index*/, void** /*ppvObj*/, int /*writeOK*/)
{
    return LXe_NOTIMPL;
}

LxResult CLxAttributesFromDesc::attr_GetInt(unsigned int index, int* val)
{
    pv_AttributeDesc::Attr* at = afd_desc->pv->lookup(index);
...
LxResult CLxAttributesFromDesc::attr_SetInt(unsigned int index, int val)
{
    pv_AttributeDesc::Attr* at = afd_desc->pv->lookup(index);
...
LxResult CLxAttributesFromDesc::attr_GetFlt(unsigned int index, double* val)
{
    pv_AttributeDesc::Attr* at = afd_desc->pv->lookup(index);
    at->p_get(afd_data, *val);
    return LXe_OK;
...
LxResult CLxAttributesFromDesc::attr_SetFlt(unsigned int index, double val)
{
    pv_AttributeDesc::Attr* at = afd_desc->pv->lookup(index);
    at->p_set(afd_data, at->clamp_range(val));
    value_changed(at->name.c_str());
...
LxResult CLxAttributesFromDesc::attr_GetString(unsigned int index, char* buf, unsigned len)
{
    pv_AttributeDesc::Attr* at = afd_desc->pv->lookup(index);
    std::string             tmp;
...
LxResult CLxAttributesFromDesc::attr_SetString(unsigned int index, const char* val)
{
    pv_AttributeDesc::Attr* at  = afd_desc->pv->lookup(index);
    std::string             tmp = val;
