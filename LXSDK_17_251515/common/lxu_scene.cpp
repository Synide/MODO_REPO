...
#define MSGTABLEs_SAVE "scene.save"
 ...
 */
static bool FileIsAbsolute(const char* fileName)
{
     ...
     */
    return (fileName[0] && fileName[1] == ':' && IS_SLASH(fileName[2])) || (fileName[2] && IS_SLASH(fileName[0]) && IS_SLASH(fileName[1]));
#else
    /*
     * Everyone else:
     *
 ...
 */
static const char* Latin_Search(const char* string, int code, bool reverse)
{
    if (reverse)
    {
...
static const char* Latin_PullChar(const char* string, int* code, int* len)
{
 ...
 */
static void NFileFromLocal(std::string& neutral, const std::string& local)
{
#if defined(_WIN32) || defined(_WIN64)
    /*
     * Windows (x86 and x64):
...
static void NFileToLocal(std::string& local, const std::string& neutral)
{
#if defined(_WIN32) || defined(_WIN64)
    /*
     * Windows (x86 and x64):
...
static char* NFileToLocal(char* local, const char* neutral)
{
 ...
 */
static bool FilePathCompose(std::string& fileName, const std::string& basename, const std::string& path)
{
    bool composed;
...
static bool FilePathCompose(char* fileName, const char* basename, const char* path)
{
    bool composed;
 ...
 * path string compare.
 */
static int FileNameCompare(const char* nameA, const char* nameB, int len)
{
#if defined(_WIN32) || defined(_WIN64)
    /*
     * Windows (x86 and x64):
...
static std::string FilePath(const std::string& baseFileName)
{
    ...
    size_t slashPos         = baseFilePath.rfind(forwardSeparator, baseFilePath.length());
    if (slashPos == std::string::npos)
    {
        char backSeparator = '\\';
        slashPos           = baseFilePath.rfind(backSeparator, baseFilePath.length());
    }
...
static void FileParseName(const std::string& filename, std::string* basename, std::string* path)
{
    std::string buf;
    ...
    int         p_base_ind = static_cast<int>(buf.rfind('/'));
    const char* p2         = Latin_Search(p_path, ':', 1);
    int         p2_ind     = static_cast<int>(buf.rfind(':'));
...
static void FileParseName(const char* filename, char* basename, char* path)
{
    std::string  fileN(filename), baseN(basename), p(path);
    std::string *baseNPtr = nullptr, *pPtr = nullptr;
...
static bool FileMakeAbsolute(std::string& absFileName, const std::string& fileName, const std::string& baseFileName)
{
    bool madeAbsolute;
    ...
    else
    {
        ...
        {
            rel = fileName.substr(2, fileName.length() - 2);
        }
        else
            rel = fileName;

        madeAbsolute = FilePathCompose(absFileName, rel, baseFilePath);
    }

    return madeAbsolute;
}

static bool FileMakeAbsolute(char* absFileName, const char* fileName, const char* baseFileName)
{
    bool madeAbsolute;
...
static bool FileMakeRelative(std::string& relname, const std::string& filename, const std::string& baseFileName)
{
    const char* p;
    std::string temp;
    int         len;
...
static bool FileMakeRelative(char* relname, const char* filename, const char* baseFileName)
{
    bool isRel;
...
namespace lx
{
    ...
    bool ComposeFilePath(const std::string& fileName, const std::string& filePath, std::string& composedPath)
    {
 ...
 */
pv_SceneBuilder::pv_SceneBuilder(ILxUnknownID obj)
{
    error = LXe_OK;
    ...
    if (!scene.set(obj) || !scene.SetChannels(chan, LXs_ACTIONLAYER_SETUP) || !scene.SetChannels(anim, LXs_ACTIONLAYER_EDIT))
    {
 ...
 */
void pv_SceneBuilder::Err(LxResult rc)
{
    if (LXx_FAIL(rc))
    {
        error = rc;
...
void pv_SceneBuilder::Err(CLxResult rc)
{
    Err(rc.resultCode());
}

void pv_SceneBuilder::Err(bool ok)
{
    if (!ok)
        Err(LXe_FAILED);
}
 ...
 */
unsigned pv_SceneBuilder::Single(int what)
{
    ...
    if (what == CLxSceneBuilder::SCENE)
    {
        if (!sScene.exists)
        {
            ...
            if (scene.GetItem(cit_scene, sceneItem))
                sScene.set(PushItem(sceneItem));
            else
                AddItem(cit_scene);
...
bool pv_SceneBuilder::IsA(const char* type)
{
    return IsA(scene_S.ItemType(type));
}

bool pv_SceneBuilder::IsA(LXtItemType type)
{
 ...
 */
unsigned pv_SceneBuilder::PushItem(ILxUnknownID obj)
{
    unsigned index;
...
ILxUnknownID pv_SceneBuilder::Item(unsigned index)
{
 ...
 */
void pv_SceneBuilder::SetItem(unsigned index)
{
    ILxUnknownID obj;
...
bool pv_SceneBuilder::FindItem(ILxUnknownID obj, unsigned& index)
{
...
unsigned pv_SceneBuilder::AddItem(LXtItemType type)
{
 ...
 */
ILxUnknownID pv_SceneBuilder::ByCue(LXtItemType type, const char* str)
{
    CLxUser_Item      cueItem;
 ...
 */
LXtPointID pv_SceneBuilder::Point(unsigned index)
{
...
LXtPolygonID pv_SceneBuilder::Polygon(unsigned index)
{
...
LXtMeshMapID pv_SceneBuilder::Map(unsigned index)
{
...
bool CLxSceneBuilder::IsGood()
{
    return pv && LXx_OK(pv->error);
}

LxResult CLxSceneBuilder::ErrorCode()
{
    return pv ? pv->error : LXe_NOACCESS;
}

void CLxSceneBuilder::Initialize(ILxUnknownID scene)
{
    try
    {
...
LxResult CLxSceneBuilder::Finalize()
{
    LxResult rc;
...
bool CLxSceneBuilder::IsValid()
{
    return (pv != nullptr && pv->scene.test());
}

unsigned CLxSceneBuilder::Singleton(int what)
{
    return pv->Single(what);
}

unsigned CLxSceneBuilder::SceneItem()
{
    return pv->Single(SCENE);
}

ILxUnknownID CLxSceneBuilder::SceneObject()
{
    return pv->scene;
}
 ...
 */
unsigned CLxSceneBuilder::AddItem(const char* itemType)
{
    LXtItemType type;
...
unsigned CLxSceneBuilder::PushItem(ILxUnknownID item)
{
    if (!IsGood())
        return 0;
...
unsigned CLxSceneBuilder::InstItem(unsigned index)
{
    if (!IsGood())
        return 0;
...
void CLxSceneBuilder::SetItem(unsigned index)
{
    if (!IsGood())
        return;
...
unsigned CLxSceneBuilder::CurItem()
{
    return pv->curIndex;
}

ILxUnknownID CLxSceneBuilder::GetItem()
{
    return pv->item;
}

bool CLxSceneBuilder::GetItem(CLxLocalizedObject& item)
{
    return item.set(pv->item);
}

bool CLxSceneBuilder::GetMesh(CLxLocalizedObject& mesh)
{
    return mesh.set(pv->mesh);
}

void CLxSceneBuilder::SetName(const char* name)
{
...
void CLxSceneBuilder::SetTag(LXtID4 type, const char* value)
{
    if (!IsGood() || !pv->GoodItem())
        return;
...
void CLxSceneBuilder::SetParent(unsigned int index, unsigned int childPos)
{
    ILxUnknownID item;
...
void CLxSceneBuilder::SetLink(const char* type, unsigned int to)
{
...
void CLxSceneBuilder::SetLink(const char* type, unsigned int to, int fromIndex, int toIndex)
{
    CLxUser_SceneGraph sgr;
    CLxUser_ItemGraph  igr;
    ILxUnknownID       iTo;
...
void CLxSceneBuilder::SetLink(const char* channelFrom, unsigned int itemTo, const char* channelTo)
{
    CLxUser_SceneGraph   sgr;
...
unsigned CLxSceneBuilder::AddRenderOutput(const char* path, const char* format, const char* effect)
{
    if (!IsGood())
        return 0;
    ...
    if (path && strlen(path) > 0)
    {
        ...
        SetChannel(LXsICHAN_RENDEROUTPUT_FORMAT, (format && strlen(format) > 0) ? format : LXsPLUGIN_IMAGESAVER_PNG8);
    }
...
bool CLxSceneBuilder::GoodMesh()
{
    return pv->GoodMesh();
}

void CLxSceneBuilder::SetChannel(const char* channel, bool value)
{
    unsigned idx;
...
void CLxSceneBuilder::SetChannel(const char* channel, int value)
{
    unsigned idx;
...
void CLxSceneBuilder::SetChannel(const char* channel, double value)
{
    unsigned idx;
...
void CLxSceneBuilder::SetChannelColor(const char* channel, const LXtVector& color)
{
    ...
    std::string channelRed   = std::string(channel) + std::string(".R");
    std::string channelGreen = std::string(channel) + std::string(".G");
    std::string channelBlue  = std::string(channel) + std::string(".B");
...
void CLxSceneBuilder::SetChannel(const char* channel, const char* value)
{
    unsigned idx;
...
void CLxSceneBuilder::SetChannelEncoded(const char* channel, const char* encodedValue)
{
     ...
     */
    const LXtTextValueHint* hints = nullptr;
    pv->item.ChannelIntHint(channelIndex, &hints);
    int value;
    pv->Err(pv->value_S.TextHintDecode(encodedValue, hints, &value));

    pv->Err(pv->chan.Set(pv->item, channelIndex, value) ? LXe_OK : LXe_FAILED);
}

void CLxSceneBuilder::SetChannel(unsigned int channel, bool value)
{
    if (!IsGood() || !pv->GoodItem())
        return;
...
void CLxSceneBuilder::SetChannel(unsigned int channel, int value)
{
    if (!IsGood() || !pv->GoodItem())
        return;
...
void CLxSceneBuilder::SetChannel(unsigned int channel, double value)
{
    if (!IsGood() || !pv->GoodItem())
        return;
...
void CLxSceneBuilder::SetChannel(unsigned int channel, const char* value)
{
    if (!IsGood() || !pv->GoodItem())
        return;
...
void CLxSceneBuilder::SetChannelEncoded(unsigned int channel, const char* encodedValue)
{
    if (!IsGood() || !pv->GoodItem())
        return;
...
void CLxSceneBuilder::SetEditTime(double frameTime)
{
    pv->Err(pv->scene.SetChannels(pv->anim, LXs_ACTIONLAYER_EDIT, frameTime) ? LXe_OK : LXe_FAILED);
}

void CLxSceneBuilder::AddEnvelope(const char* channel, CLxUser_Envelope& env)
{
    unsigned idx;
...
void CLxSceneBuilder::AddEnvelope(unsigned int channel, CLxUser_Envelope& env)
{
    if (!IsGood() || !pv->GoodItem())
        return;
...
void CLxSceneBuilder::AddValue(const char* channel, CLxUser_Value& value)
{
...
void CLxSceneBuilder::AddValue(unsigned int channel, CLxUser_Value& value)
{
    LXtObjectID obj = nullptr;
...
void CLxSceneBuilder::SetXform(LXtTransformType type, double vec[3])
{
    if (!IsGood() || !pv->GoodItem())
        return;
...
void CLxSceneBuilder::SetXform(LXtTransformType type, double x, double y, double z)
{
    double vec[3] = { x, y, z };
...
void CLxSceneBuilder::SetXform(LXtTransformType type, double val)
{
...
unsigned CLxSceneBuilder::AddXform(LXtTransformType type, unsigned* index)
{
    LXtObjectID obj;
...
unsigned CLxSceneBuilder::AddPreXform(LXtTransformType type, double vec[3], unsigned* index)
{
    if (!IsGood() || !pv->GoodItem())
        return 0;
...
unsigned CLxSceneBuilder::AddPreXform(LXtTransformType type, double x, double y, double z, unsigned* index)
{
    double vec[3] = { x, y, z };
...
unsigned CLxSceneBuilder::AddPreXform(LXtTransformType type, double val, unsigned* index)
{
    double vec[3] = { val, val, val };
...
unsigned CLxSceneBuilder::AddPostXform(LXtTransformType type, double vec[3], unsigned* index)
{
    if (!IsGood() || !pv->GoodItem())
        return 0;
...
unsigned CLxSceneBuilder::AddPostXform(LXtTransformType type, double x, double y, double z, unsigned* index)
{
    double vec[3] = { x, y, z };
...
unsigned CLxSceneBuilder::AddPostXform(LXtTransformType type, double val, unsigned* index)
{
    double vec[3] = { val, val, val };
...
unsigned CLxSceneBuilder::PrependXform(LXtTransformType type, double vec[3], unsigned* index)
{
    if (!IsGood() || !pv->GoodItem())
        return 0;
...
unsigned CLxSceneBuilder::PrependXform(LXtTransformType type, double x, double y, double z, unsigned* index)
{
    double vec[3] = { x, y, z };
...
unsigned CLxSceneBuilder::PrependXform(LXtTransformType type, double val, unsigned* index)
{
    double vec[3] = { val, val, val };
...
unsigned CLxSceneBuilder::AppendXform(LXtTransformType type, double vec[3], unsigned* index)
{
    if (!IsGood() || !pv->GoodItem())
        return 0;
...
unsigned CLxSceneBuilder::AppendXform(LXtTransformType type, double x, double y, double z, unsigned* index)
{
    double vec[3] = { x, y, z };
...
unsigned CLxSceneBuilder::AppendXform(LXtTransformType type, double val, unsigned* index)
{
    double vec[3] = { val, val, val };
...
void CLxSceneBuilder::AddPackage(const char* name)
{
    pv->Err(pv->item.PackageAdd(name));
}

unsigned CLxSceneBuilder::RenderItem()
{
 ...
 */
unsigned CLxSceneBuilder::AddTexture(const char* type, int where)
{
    int whereIdx = 0;
...
unsigned CLxSceneBuilder::AddMaterial(const char* matName, int where, const char* matType)
{
    unsigned msk;
...
unsigned CLxSceneBuilder::AddImageFolder(const char* /*folderName*/)
{
    return AddItem(LXsITYPE_IMAGEFOLDER);
}

void CLxSceneBuilder::SpecExponent(double exp)
{
    if (exp > 0.0)
...
unsigned CLxSceneBuilder::AddImage(const char* filename)
{
    ILxUnknownID clip;
    LxResult     rc;
    ...
    rc = pv->scene_S.LoadImage(pv->scene, filename, nullptr, (void**) &clip);
    if (rc == LXe_NOTFOUND)
        return 0;
...
unsigned CLxSceneBuilder::AddImageMap(const char* filename, const char* effect, const char* uvmap, int where)
{
...
unsigned int CLxSceneBuilder::TxtrLocator()
{
...
unsigned CLxSceneBuilder::AddMesh(bool asStatic)
{
    LXtObjectID  mesh;
    ...
    if (!pv->mesh.take(mesh) || !pv->mesh.GetPoints(pv->point) || !pv->mesh.GetPolygons(pv->poly) || !pv->mesh.GetMaps(pv->mmap) ||
...
unsigned CLxSceneBuilder::ConvertMesh()
{
    if (!IsGood() || !GoodMesh())
        return 0;
...
unsigned int CLxSceneBuilder::AddPoint(LXtVector pos)
{
    LXtPointID pnt;
...
unsigned int CLxSceneBuilder::AddPoint(LXtFVector pos)
{
    LXtVector fv;
...
unsigned int CLxSceneBuilder::AddPoint(double x, double y, double z)
{
    LXtVector fv;
...
void CLxSceneBuilder::StartPoly(LXtID4 type)
{
    if (!pv->GoodMesh())
        return;
...
void CLxSceneBuilder::AddVertex(unsigned index)
{
    LXtPointID pnt;
...
unsigned int CLxSceneBuilder::AddPolygon(bool rev)
{
    LXtPolygonID poly;
    int          i, n;
    LXtPointID*  vrts;
...
void CLxSceneBuilder::SetPolyTag(unsigned index, LXtID4 type, const char* value)
{
    LXtPolygonID pol;
...
unsigned int CLxSceneBuilder::AddMap(LXtID4 type, const char* name)
{
    LXtMeshMapID mmap;
...
bool CLxSceneBuilder::SetMap(LXtID4 type, const char* name)
{
    if (name)
    ...
    pv->mmap.Enum(&vis);
    return pv->mmap.ID() != nullptr;
}

void CLxSceneBuilder::SetPointMap(unsigned mapIndex, float* vec, unsigned pntIndex)
{
    LXtMeshMapID mmap;
    LXtPointID   pnt;
...
void CLxSceneBuilder::SetVertexMap(unsigned mapIndex, float* vec, unsigned pntIndex, unsigned polIndex)
{
    LXtMeshMapID mmap;
    LXtPointID   pnt;
    LXtPolygonID pol;
...
void CLxSceneBuilder::SetCoVertMap(unsigned mapIndex, float* vec, unsigned pntIndex, unsigned polIndex)
{
    LXtMeshMapID mmap;
    LXtPointID   pnt;
    LXtPolygonID pol;
...
void CLxSceneBuilder::SetEdgeMap(const float* vec, unsigned p1Index, unsigned p2Index)
{
    if (!pv->GoodMesh())
        return;
...
unsigned CLxSceneBuilder::AddMeshOp(const char* type)
{
    unsigned int sceneIndex  = pv->Single(SCENE);
    unsigned int index       = std::numeric_limits<unsigned>::max();
    unsigned int targetIndex = std::numeric_limits<unsigned>::max();
...
unsigned CLxSceneBuilder::AddSelOp(const char* type)
{
    unsigned int index       = std::numeric_limits<unsigned>::max();
    unsigned int targetIndex = std::numeric_limits<unsigned>::max();
...
LxResult CLxSceneLoader::load_Recognize(const char* filename, ILxUnknownID ldInfo)
{
...
LxResult CLxSceneLoader::load_LoadObject(ILxUnknownID /*ldInfo*/, ILxUnknownID monitor, ILxUnknownID dest)
{
    CLxFileParser* par = sl_Parser();
...
void CLxSceneLoader::load_Cleanup()
{
    ld_target.clear();
    scene_build.Cleanup();
...
LxResult CLxSceneLoader::load_SpawnOptions(void** ppvObj)
{
    sl_SpawnOpt(ppvObj);
    return LXe_OK;
}

bool CLxSceneLoader::IsFileAbsolute(const char* fileName) const
{
    return FileIsAbsolute(fileName);
}

bool CLxSceneLoader::IsFileAbsolute(const std::string& fileName) const
{
    return IsFileAbsolute(fileName.c_str());
}

bool CLxSceneLoader::MakeFileAbsolute(char* absFileName, const char* fileName)
{
    /*
     * file_name is the base file name to the scene, whereas fileName
     * is the file name for the referenced asset, such as an image.
     */
    return FileMakeAbsolute(absFileName, fileName, sl_Parser()->file_name);
}

bool CLxSceneLoader::MakeFileAbsolute(std::string& fileName)
{
     ...
     * is the file name for the referenced asset, such as an image.
     */
    bool result = FileMakeAbsolute(absFileNameChars, fileName, std::string(sl_Parser()->file_name));
...
public:
    ...
    bool CollectLayers(CLxUser_Item&, const char*, const char* = nullptr);
    ...
    LXtItemType            meshType, meshInstType, triSurfType, rendType, maskType, tlayType, stilType, tlocType;
 ...
 */
pv_SceneSaver::pv_SceneSaver(ILxUnknownID obj, bool subsets) : real(false)
{
    ...
    if (scene.test() && scene.GetChannels(chan, LXs_ACTIONLAYER_EDIT) && scene.GetChannels(chanXForm, 0.0) && scene.GetGraph(LXsGRAPH_SHADELOC, gr) &&
        sGraph.set(gr))
    {
        error = LXe_OK;
    }
 ...
 */
bool pv_SceneSaver::Err(LxResult rc) const
{
    if (LXx_OK(rc))
        return false;
 ...
 */
LxResult pv_SceneSaver::LastErr() const
{
    return error;
}
 ...
 * check for abort.
 */
void pv_SceneSaver::Step(unsigned n)
{
    if (!real)
        steps += n;
 ...
 */
bool pv_SceneSaver::Tally(unsigned n)
{
    if (real)
        return false;
 ...
 */
bool pv_SceneSaver::Test(ILxUnknownID testItem)
{
    if (icoll.test())
        return (icoll.Test(testItem) == LXe_TRUE);
 ...
 */
bool pv_SceneSaver::CollectLayers(CLxUser_Item& root, const char* polyMask, const char* itemMask)
{
    CLxUser_Item sub;
    const char*  ptag;
    unsigned     idx, i, n;
    ...
    for (i = 0; i < n; i++)
    {
         ...
         */
        bool         itemTagMatch(itemMask == nullptr);
        ...
        if (itemCount && itemGraph.Forward(sub, 0, layerMask))
        {
            if (LXx_OK(layerMask.UniqueName(&layerName)))
            {
                if (itemMask)
                {
                    itemTagMatch = layerName && !strcmp(layerName, itemMask);
                }
                else
                {
                    /*
                     ...
                     */
                    itemTagMatch = (layerName == nullptr) || ((!strlen(ptag)) && !itemMask);
                }
            }
        }
        if (!itemTagMatch)
...
bool pv_SceneSaver::UpdateMeshAccessors()
{
    return mesh.GetPoints(point) && mesh.GetEdges(edge) && mesh.GetPolygons(poly) && mesh.GetMaps(map) && polyTag.set(poly);
}
...
/*
 * Verification --  Count the number of triangle, and not the number of polygon
 * for the NC limitation
 */

class CTriCountVisitor : public CLxImpl_AbstractVisitor
{
public:
    CLxSceneSaver* ss;
    unsigned       ntri;
    LxResult       Evaluate() override
    {
        ...
        // For curve, count polygon as 1 triangle
        LXtID4 type;
        ss->pv->poly.Type(&type);
        switch (type)
        {
            case LXiPTYP_CURVE:
            case LXiPTYP_BEZIER:
            case LXiPTYP_LINE:
            case LXiPTYP_BSPLINE:
                return LXe_OK;
        }

        // Else triangle number is 2 less the vertex number
        unsigned nvrt;
        ss->pv->poly.VertexCount(&nvrt);
        ntri += nvrt > 3 ? nvrt - 3 : 0;  // We already count 1

        return LXe_OK;
    }
};

unsigned TriangleCount(CLxSceneSaver* ss)
{
    if (!ss->pv->item.IsA(ss->pv->meshType) && ss->pv->item.IsA(ss->pv->meshInstType))
        return 0;

    CTriCountVisitor vis;
    vis.ss   = ss;
    vis.ntri = 0;
    ss->pv->poly.Enum(&vis);
    return vis.ntri;
 ...
 */
LxResult CLxSceneSaver::sav_Verify(ILxUnknownID source, ILxUnknownID msg)
{
    ...
    else if (!pv->msg.set(msg))
        throw LXe_INVALIDARG;

    try
    {
        unsigned int triangle_limit = pv->scene.TriangleLimit();
        if (triangle_limit < std::numeric_limits<unsigned int>::max())
        {
            unsigned int exported_triangles = 0;
            ...
            while (NextMesh())
            {
                CLxUser_Item item;
                GetItem(item);
                if (item.test() && ss_ItemValidForExport(item))
                    exported_triangles += TriangleCount(this);
            }

            if (exported_triangles > triangle_limit)
            {
                pv->msg.SetCode(LXe_DISABLED);
                pv->msg.SetMessage(MSGTABLEs_SAVE, "polyLimitError", 0);
                pv->msg.SetArgumentString(1, CLxUser_PlatformService().AppUsername().c_str());
                pv->msg.SetArgumentInt(2, triangle_limit);
                pv->msg.SetArgumentInt(3, exported_triangles);
                throw LXe_DISABLED;
            }
        }
...
void CLxSceneSaver::Message(const char* table, unsigned id)
{
    pv->msg.SetCode(LXe_INFO);
    pv->msg.SetMessage(table, nullptr, id);
}

void CLxSceneSaver::Message(const char* table, const char* key)
{
    pv->msg.SetCode(LXe_INFO);
    pv->msg.SetMessage(table, key, 0);
}

void CLxSceneSaver::MessageArg(unsigned arg, int value)
{
    pv->msg.SetArg(arg, value);
}

void CLxSceneSaver::MessageArg(unsigned arg, double value)
{
    pv->msg.SetArg(arg, value);
}

void CLxSceneSaver::MessageArg(unsigned arg, const char* value)
{
    pv->msg.SetArg(arg, value);
}
 ...
 */
LxResult CLxSceneSaver::sav_Save(ILxUnknownID source, const char* filename, ILxUnknownID mon)
{
 ...
 */
ILxUnknownID CLxSceneSaver::SceneObject()
{
    return pv->scene;
}

bool CLxSceneSaver::ReallySaving()
{
    return pv->real;
}

bool CLxSceneSaver::TestItem(ILxUnknownID item)
{
    return pv->Test(item);
}

bool CLxSceneSaver::IsSceneSaved()
{
    const char* value;
    LxResult    rc;
...
const char* CLxSceneSaver::ScenePath()
{
    const char* value;
    LxResult    rc;
...
const char* CLxSceneSaver::SceneName(unsigned flags)
{
 ...
 */
void CLxSceneSaver::StartScan(const char* typeName)
{
    pv->scanMode = SM_NONE;
    if (typeName)
 ...
 */
bool CLxSceneSaver::NextSelectedMesh()
{
    bool ok = false;
...
bool CLxSceneSaver::GetMesh(CLxUser_Mesh& mesh)
{
    return mesh.set(pv->mesh);
}

bool CLxSceneSaver::GetPolygon(CLxUser_Polygon& poly)
{
 ...
 */
bool CLxSceneSaver::ScanMask(const char* polyMask, const char* itemMask)
{
    CLxUser_Item rend;
...
bool CLxSceneSaver::NextLayer()
{
    LXtObjectID obj;

    //	pv->Step ();

    if (pv->scanMode != SM_LAYER && !ScanMask(nullptr))
        return false;
 ...
 */
bool CLxSceneSaver::SetItem(ILxUnknownID obj)
{
    ...
    if (pv->item.set(obj))
    {
        ...
        else if (pv->item.IsA(pv->meshInstType))
        {
            ...
            ILxItem1ID           iMesh;
            if (LXx_OK(service.GetMeshInstSourceItem((ILxUnknownID) pv->item, (void**) &iMesh)))
            {
                ...
                if (LXe_TRUE == iMesh[0]->TestType(iMesh, LXsITYPE_TRISURF))
                    isItemSet = true;
                else if (LXx_OK(iMesh[0]->ChannelIndex(iMesh, LXsICHAN_MESH_MESH, &index)))
                {
                    LXtObjectID meshObj;
                    if (LXx_OK(pv->chan.m_loc[0]->ValueObj(pv->chan, iMesh, index, (void**) &meshObj)))
                    {
                        isItemSet = haveMesh = pv->mesh.take(meshObj);
                    }
                }
            }
        }
 ...
 */
bool CLxSceneSaver::SetMeshTime(double frameTime)
{
    ...
    if (pv->item.IsA(pv->meshInstType))
    {
        ...
        if (LXx_FAIL(service.GetMeshInstSourceItem(pv->item, (void**) &iMesh)) || false == srcMeshItem.take(iMesh))
            return false;

        // Handle instances with allowed deformation on
        if (ChanBool(LXsICHAN_MESHINST_ALLOWDEFORM))
        {
            if (0 > (index = pv->item.ChannelIndex(LXsICHAN_MESH_MESH)) || false == fChan.Object(pv->item, index, pv->meshFilter))
                return false;
        }
        else
        {
            if (LXe_TRUE == iMesh[0]->TestType(iMesh, LXsITYPE_TRISURF) || LXx_FAIL(iMesh[0]->ChannelIndex(iMesh, LXsICHAN_MESH_MESH, &uindex)) ||
    ...
    else if (0 > (index = pv->item.ChannelIndex(LXsICHAN_MESH_MESH)) || false == fChan.Object(pv->item, index, pv->meshFilter))
        return false;

    return pv->meshFilter.GetMesh(pv->mesh) && pv->UpdateMeshAccessors();
}

bool CLxSceneSaver::GetItem(CLxUser_Item& item) const
{
    item.clear();
    if (pv->item.test())
        item.set(pv->item);
 ...
 */
const char* CLxSceneSaver::ItemName() const
{
    const char* name;
...
std::string CLxSceneSaver::ItemIdentity() const
{
    return pv->item.GetIdentity();
}

const char* CLxSceneSaver::ItemType() const
{
    const char* name;
...
LXtItemType CLxSceneSaver::GetItemType() const
{
    return pv->item.Type();
}

LXtItemType CLxSceneSaver::ItemType(const char* typeName) const
{
...
bool CLxSceneSaver::ItemIsA(const char* typeName) const
{
    LXtItemType type;
...
const char* CLxSceneSaver::ItemTag(LXtID4 tag)
{
    const char* value;
...
unsigned CLxSceneSaver::ItemTagCount()
{
    unsigned count;
...
const char* CLxSceneSaver::ItemTagByIndex(unsigned index, LXtID4* tag)
{
    const char* value;
...
bool CLxSceneSaver::ItemVisible() const
{
    bool visible = true;
 ...
 * items.
 */
bool CLxSceneSaver::WorldXform(LXtMatrix xfrm, LXtVector offset)
{
    bool            result = false;
    CLxUser_Locator locator;
    if (locator.set(pv->item))
...
bool CLxSceneSaver::HasXformItem(LXtTransformType type) const
{
    LXtObjectID obj;
    bool        hasXform = false;
...
bool CLxSceneSaver::XformItem(LXtTransformType type)
{
    LXtObjectID obj;
    bool        hasXform = false;
 ...
 * their attributes.
 */
const char* CLxSceneSaver::LayerEffect()
{
    return ChanString(LXsICHAN_TEXTURELAYER_EFFECT);
}

double CLxSceneSaver::SpecExponent()
{
    double rough;
...
bool CLxSceneSaver::TxtrLocator()
{
    CLxUser_Item sub;
    int          i, n;
 ...
 */
bool CLxSceneSaver::ChanBool(unsigned index) const
{
    int value = 0;
...
int CLxSceneSaver::ChanInt(unsigned index) const
{
    int value = 0;
...
double CLxSceneSaver::ChanFloat(unsigned index) const
{
    double value = 0.0;
...
const char* CLxSceneSaver::ChanString(unsigned index) const
{
    const char* value = nullptr;
...
bool CLxSceneSaver::ChanObject(unsigned index, CLxLocalizedObject& loc) const
{
    return pv->chan.Object(pv->item, index, loc);
}

bool CLxSceneSaver::ChanEnvelope(unsigned index, CLxUser_Envelope& env) const
{
    return pv->chan.GetEnv(pv->item, index, env);
}

bool CLxSceneSaver::ChanIsAnimated(unsigned index) const
{
    // we need to use the time-based channel reader
    return 0 != pv->chanXForm.IsAnimated(pv->item, index);
}

unsigned CLxSceneSaver::ChanIndex(const char* channel) const
{
    unsigned index;
...
bool CLxSceneSaver::ChanBool(const char* channel) const
{
    unsigned index;
...
int CLxSceneSaver::ChanInt(const char* channel) const
{
    unsigned index;
...
double CLxSceneSaver::ChanFloat(const char* channel) const
{
    unsigned index;
...
void CLxSceneSaver::ChanColor(const char* channel, LXtVector vec) const
{
    ...
    resultR = pv->item.ChannelLookup(std::string(std::string(channel) + std::string(CHANNEL_R)).c_str(), &indexR);
    resultG = pv->item.ChannelLookup(std::string(std::string(channel) + std::string(CHANNEL_G)).c_str(), &indexG);
    resultB = pv->item.ChannelLookup(std::string(std::string(channel) + std::string(CHANNEL_B)).c_str(), &indexB);
...
void CLxSceneSaver::ChanXform(const char* channel, LXtVector vec) const
{
    ...
    resultX = pv->item.ChannelLookup(std::string(std::string(channel) + std::string(CHANNEL_X)).c_str(), &indexX);
    resultY = pv->item.ChannelLookup(std::string(std::string(channel) + std::string(CHANNEL_Y)).c_str(), &indexY);
    resultZ = pv->item.ChannelLookup(std::string(std::string(channel) + std::string(CHANNEL_Z)).c_str(), &indexZ);
...
const char* CLxSceneSaver::ChanString(const char* channel) const
{
    unsigned index;
...
void CLxSceneSaver::ChanIntEncoded(unsigned channelIndex, char* buf, int bufLen) const
{
    /*
    ...
    pv->item.ChannelIntHint(channelIndex, &hints);
    buf[0] = 0;
    pv->Err(valueSvc.TextHintEncode(channelValue, hints, buf, bufLen));
}

void CLxSceneSaver::ChanIntEncoded(const char* channel, char* buf, int bufLen) const
{
    unsigned index;
...
bool CLxSceneSaver::ChanObject(const char* channel, CLxLocalizedObject& loc) const
{
    unsigned index;
...
bool CLxSceneSaver::ChanEnvelope(const char* channel, CLxUser_Envelope& env) const
{
    unsigned index;
...
bool CLxSceneSaver::ChanIsAnimated(const char* channel) const
{
    unsigned index;
...
bool CLxSceneSaver::IsFileAbsolute(const char* fileName) const
{
    return FileIsAbsolute(fileName);
}

bool CLxSceneSaver::IsFileAbsolute(const std::string& fileName) const
{
    return IsFileAbsolute(fileName.c_str());
}

bool CLxSceneSaver::MakeFileRelative(char* relName, const char* fileName)
{
...
bool CLxSceneSaver::MakeFileRelative(std::string& fileName)
{
    bool madeRelative(false);
...
bool CLxSceneSaver::MakeFileAbsolute(char* absFileName, const char* fileName)
{
    /*
     * file_name is the base file name to the scene, whereas fileName
     * is the file name for the referenced asset, such as an image.
...
bool CLxSceneSaver::MakeFileAbsolute(std::string& fileName)
{
     ...
     * is the file name for the referenced asset, such as an image.
     */
    bool result = FileMakeAbsolute(absFileNameChars, fileName.c_str(), ss_Format()->file_name);
 ...
 */
void CLxSceneSaver::MeshBounds(LXtBBox& box)
{
    pv->Err(pv->mesh.BoundingBox(LXiMARK_ANY, &box));
}

bool CLxSceneSaver::SetMap(LXtID4 type, const char* name)
{
    if (name)
    ...
    pv->map.Enum(&vis);
    return pv->map.ID() != nullptr;
}

bool CLxSceneSaver::SetSelMap(LXtID4 type)
{
 ...
 */
unsigned CLxSceneSaver::PointCount()
{
    unsigned count = 0;
    ...
    else if (pv->item.IsA(pv->triSurfType))
    {
        ...
        do
        {
            unsigned vrtCount;
            unsigned triCount;
            nextTriSurf = pv->triangleGroup.GetSurf(triSurfIndex++, pv->triangleSurface);
            if (nextTriSurf)
            {
...
static LxResult TriangleSurfaceVectorIndex(CLxUser_TriangleSurface& triangleSurface, LXtID4 vectorType, unsigned& vectorIndex)
{
    ...
    vectorIndex = 0;
    while (LXx_OK(triangleSurface.VectorInfo(vectorIndex, &testVectorType, &vectorName, &stride)))
    {
...
void CLxSceneSaver::WritePoints()
{
    CPointVisitor vis;
    ...
    else if (pv->item.IsA(pv->triSurfType))
    {
        ...
        do
        {
            ...
            {
                pv->triangleSurface.GetSize(&vrtCount, &triCount);
                if (LXx_OK(TriangleSurfaceVectorIndex(pv->triangleSurface, LXi_VMAP_OBJECTPOS, pv->posVectorIndex)))
                {
                    //	float *points = pv->triangleSurface.Vector (
                    //		pv->posVectorIndex);
                    for (pv->trianglePointIndex = 0; pv->trianglePointIndex < vrtCount; ++pv->trianglePointIndex)
                    {
                        vis.Evaluate();
                    }
                }
...
void CLxSceneSaver::PntSet(LXtPointID pnt)
{
    pv->Err(pv->point.Select(pnt));
}

LXtPointID CLxSceneSaver::PntID()
{
    return pv->point.ID();
}

void CLxSceneSaver::PntPosition(LXtFVector vec)
{
    if (pv->item.IsA(pv->meshType) || pv->item.IsA(pv->meshInstType))
    {
        pv->point.Pos(vec);
...
void CLxSceneSaver::PntPosition(LXtVector vec)
{
    if (pv->item.IsA(pv->meshType) || pv->item.IsA(pv->meshInstType))
    {
        LXtFVector fv;
...
bool CLxSceneSaver::PntNormal(LXtVector norm, LXtPolygonID pol)
{
    return LXx_OK(pv->point.Normal(pol, norm));
}

bool CLxSceneSaver::PntMapValue(float* vec)
{
 ...
 */
unsigned CLxSceneSaver::EdgeCount()
{
    unsigned count = 0;
...
LXtEdgeID CLxSceneSaver::EdgeID()
{
    return pv->edge.ID();
}

bool CLxSceneSaver::EdgeEndPoints(LXtPointID* point1, LXtPointID* point2)
{
    LxResult result = pv->edge.Endpoints(point1, point2);
    return (LXx_OK(result) && (result != LXe_FALSE));
}

bool CLxSceneSaver::EdgeMapValue(float* vec)
{
 ...
 * Polygon Enumeration
 */
unsigned CLxSceneSaver::PolygonCount()
{
    unsigned count = 0;
...
public:
    ...
    CPolyVisitor() : unifyByPolyMatTags(false), meshMaterialMap(nullMap)
    {
    }

    CPolyVisitor(MeshMaterialMap& map) : unifyByPolyMatTags(true), meshMaterialMap(map)
    {
    }
    ...
    LxResult Evaluate() override
    {
        ...
        {
            const PolyMaterialTag polyMatTag(ss->PolyTag(LXi_PTAG_MATR));
            LXtPolygonID          polyID(ss->PolyID());
            meshMaterialMap.insert(std::make_pair(polyMatTag, polyID));
        }
        else
        {
            ss->pv->Step();
...
void CLxSceneSaver::WritePolys(LXtID4 type, bool unifyByPolyMatTags)
{
    ...
    if (unifyByPolyMatTags)
    {
         ...
         */
        for (MeshMaterialMap::const_iterator iter = meshMaterialMap.begin(); iter != meshMaterialMap.end(); ++iter)
        {
            PolySet(iter->second);
            ss_Polygon();
        }
...
void CLxSceneSaver::PolySet(LXtPolygonID pol)
{
    pv->Err(pv->poly.Select(pol));
}

LXtPolygonID CLxSceneSaver::PolyID()
{
    return pv->poly.ID();
}

LXtID4 CLxSceneSaver::PolyType()
{
    LXtID4 t;
...
unsigned CLxSceneSaver::PolyNumVerts()
{
    unsigned n = 0;
...
LXtPointID CLxSceneSaver::PolyVertex(unsigned index)
{
    LXtPointID pnt;
...
const char* CLxSceneSaver::PolyTag(LXtID4 tag)
{
    const char* value;
...
bool CLxSceneSaver::PolyNormal(LXtVector norm, LXtPointID vrt)
{
    if (!vrt)
...
bool CLxSceneSaver::PolyMapValue(float* vec, LXtPointID vrt)
{
    LxResult rc;
...
LxResult CLxSceneSaver::LastErr() const
{
    return pv->LastErr();
}

void CLxSceneSaver::Step(unsigned n)
{
    pv->Step(n);
}

void CLxSceneSaver::Tally(unsigned n)
{
    pv->Tally(n);
}
