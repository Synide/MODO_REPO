...
public:
    ...
    CLxMeta_ValueTexture_Core* m_core;
 ...
 */
class meta_ValueTextureServer : public CLxImpl_ValueTexture, public CLxImpl_TagDescription
{
public:
 ...
 */
class meta_ValueTexturePolymorph : public CLxPolymorph<meta_ValueTextureServer>
{
public:
    class pv_Meta_ValueTexture* pv;

    meta_ValueTexturePolymorph(class pv_Meta_ValueTexture* ptr)
    {
 ...
 */
CLxMeta_ValueTexture_Core::CLxMeta_ValueTexture_Core(const char* srvName) : CLxMetaServer(srvName)
{
...
bool CLxMeta_ValueTexture_Core::pre_init()
{
    CLxMeta* m;
...
void* CLxMeta_ValueTexture_Core::alloc()
{
...
const char* CLxMeta_ValueTexture_Core::item_type()
{
...
public:
    CLxTagList*                tag_list;
    CLxMeta_VectorPacket_Core* m_core;
 ...
 * the metaclass here.
 */
class meta_PacketEffect_Server : public CLxImpl_PacketEffect, public CLxImpl_TagDescription
{
public:
    pv_PacketEffect* pv;

    meta_PacketEffect_Server(pv_PacketEffect* init = nullptr)
    {
        pv = init;
    }
    ...
    {
        return static_cast<unsigned>(pv->vp_pv->pfx_list.size());
    }
    ...
    template <class T>
    T* struct_element(pfxEffect& pfx, void* packet)
    {
        char* b;
...
class meta_PacketEffect : public CLxMetaServer
{
public:
    pv_PacketEffect* pv;

    meta_PacketEffect(const char* srvName, pv_Meta_VectorPacket* vp) : CLxMetaServer(srvName)
    {
        pv           = &vp->pe_pv;
        pv->tag_list = &tag_list;
    ...
    void* alloc() override
    {
        return reinterpret_cast<void*>(new CLxMetagenPolymorph<meta_PacketEffect_Server, pv_PacketEffect>(pv));
    }
};
 ...
 */
class meta_VectorPacket_Server : public CLxImpl_VectorPacket, public CLxImpl_TagDescription
{
public:
    pv_Meta_VectorPacket* pv;

    meta_VectorPacket_Server(pv_Meta_VectorPacket* init = nullptr)
    {
        pv = init;
    }
    ...
    {
        return static_cast<unsigned>(pv->v_type->p_size());
    }
 ...
 */
void CLxVectorPacket_Void::add_effect(const char* name, const char* type, int flags)
{
    pfxEffect pfx;
...
void CLxVectorPacket_Void::struct_offset(float* offset)
{
 ...
 */
CLxMeta_VectorPacket_Core::CLxMeta_VectorPacket_Core(const char* srvName) : CLxMetaServer(srvName)
{
...
void CLxMeta_VectorPacket_Core::self_init()
{
    if (pv->v_type)
        return;
...
void CLxMeta_VectorPacket_Core::packet_category(const char* cat)
{
    self_init();
    add_tag(LXsVPK_CATEGORY, cat);
}

void CLxMeta_VectorPacket_Core::effect_category(const char* cat)
{
    self_init();
    pv->pfx_cat = cat;
}

bool CLxMeta_VectorPacket_Core::pre_init()
{
    self_init();
...
void* CLxMeta_VectorPacket_Core::alloc()
{
    self_init();

    return reinterpret_cast<void*>(new CLxMetagenPolymorph<meta_VectorPacket_Server, pv_Meta_VectorPacket>(pv));
}
