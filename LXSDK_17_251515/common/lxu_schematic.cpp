...
public:
    CLxTagList*                       tag_list;
    CLxMeta_SchematicConnection_Core* m_core;
    CLxList<pv_SchematicConnection>   inst_list;
...
public:
    ...
    std::vector<CLxUser_Item>    item_list;
    int                          tmp_flags;
    bool                         flags_valid;
    ...
    void set_parent(pv_Meta_SchematicConnection* parent)
    {
        pv = parent;
 ...
 * Tags interfaces.
 */
class meta_SchematicConnectionServer : public CLxImpl_SchematicConnection, public CLxImpl_TagDescription
{
public:
    ...
    void refresh_list(ILxUnknownID itemObj)
    {
    ...
    LXxO_SchematicConnection_Count
    {
        ...
        refresh_list(item);
        count[0] = static_cast<unsigned>(sc->pv->item_list.size());
        return LXe_OK;
    }
    ...
    {
        return (pv->b_reverse ? LXfSCON_REVERSE : 0) | (pv->b_manual ? LXfSCON_USESERVER : 0) | (pv->b_dynamic ? LXfSCON_PERITEM : 0);
    }
...
void CLxSchematicConnection::set_single()
{
    pv->tmp_flags |= LXfSCON_SINGLE;
}

void CLxSchematicConnection::set_multiple(bool ordered)
{
...
void CLxSchematicConnection::add_item(CLxUser_Item& item)
{
 ...
 */
class meta_SchematicConnectionPolymorph : public CLxPolymorph<meta_SchematicConnectionServer>
{
public:
    class pv_Meta_SchematicConnection* pv;

    meta_SchematicConnectionPolymorph(class pv_Meta_SchematicConnection* ptr)
    {
 ...
 */
CLxMeta_SchematicConnection_Core::CLxMeta_SchematicConnection_Core(const char* srvName) : CLxMetaServer(srvName)
{
...
void* CLxMeta_SchematicConnection_Core::alloc()
{
    return reinterpret_cast<void*>(new meta_SchematicConnectionPolymorph(pv));
}

void CLxMeta_SchematicConnection_Core::set_graph(const char* graph, bool reverse, bool manual)
{
    pv->graph_name = graph;
...
void CLxMeta_SchematicConnection_Core::set_itemtype(const char* typeName, bool multiple, bool ordered)
{
    pv->item_type  = typeName;
    pv->b_bytype   = true;
    pv->b_dynamic  = false;
    pv->type_flags = (multiple ? LXfSCON_MULTIPLE : LXfSCON_SINGLE) | (ordered ? LXfSCON_ORDERED : 0);
}

void CLxMeta_SchematicConnection_Core::set_dynamic()
{
    pv->b_dynamic = true;
    pv->b_bytype  = false;
}

void CLxMeta_SchematicConnection_Core::invalidate()
{
